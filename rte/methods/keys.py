...
def _canon(p: dict) -> dict:
    ...
    if audit is True or audit == 0.05:
        p.pop("audit", None)
    elif not audit:
        p["audit"] = False
    ...
    if verify:
        ...
        p["verify"] = False
    if "cached" in p and bool(p["cached"]) == verify:
        p.pop("cached")
    return p
...
def to_new(method: str, params: dict | None) -> tuple[str, dict] | None:
    ...
    if method in WITHDRAWN:
        return None
    if method == "midian_va":
        return "midian", _canon(p)
    if method == "midian_a":
        return "midian", _canon({**p, "verify": False})
    if method == "midian_v":
        for k in ("verify", "cached"):
            p.pop(k, None)
...
def normalize(df, results_dir: str):
    ...
    if os.path.exists(os.path.join(results_dir, SENTINEL)):
        for m, p in set(old):
            assert_v2(m, load(p), results_dir)
        return df
    new = {mp: to_new(mp[0], load(mp[1])) for mp in set(old)}
    if all(v is not None and v[0] == mp[0] and _jkey(v[1]) == _jkey(load(mp[1])) for mp, v in new.items()):
        return df
    keep = [new[mp] is not None for mp in old]
    df = df[keep].copy()
    kept = [mp for mp, k in zip(old, keep) if k]
    df["method"] = [new[mp][0] for mp in kept]
    if "params" in df:
        df["params"] = [_jkey(new[mp][1]) for mp in kept]
    return df

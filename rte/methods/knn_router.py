...
class KNNRouter(Method):
    ...
    def build(self, view, budget):
        ...
        self.cnt = np.full(view.n, self.E.shape[1])
        self.k = self.k or budget.b
    ...
    def observe(self, task, agent, outcome):
        ...
        if self.cnt[agent] == self.E.shape[1]:                         # grow the store
            self.E = np.concatenate([self.E, np.zeros_like(self.E)], 1)
            self.Y = np.concatenate([self.Y, np.zeros_like(self.Y)], 1)
        self.E[agent, self.cnt[agent]], self.Y[agent, self.cnt[agent]] = self._q, outcome
        self.cnt[agent] += 1

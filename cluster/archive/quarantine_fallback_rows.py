...
A row is contaminated when its supervisor never really answered and the adapter routed by declared argmax instead:
  * CrewAI / ADK (envs with deleted .so files): fallback > 0.05 -- their working units sit at ~0, the failure is per
  unit
...
for gdir in sorted(glob.glob(f"{R}/*")):
    if not os.path.basename(gdir).startswith(FW_PREFIXES):
        continue
    if not (os.path.isdir(f"{gdir}/rows.d") or os.path.exists(f"{gdir}/rows.csv")):
        continue
    ...
    for f in glob.glob(f"{gdir}/rows.d/*.json"):
        ...
        if contaminated(str(r.get("method")), r.get("method_stats")):
            ...
            if APPLY:
                ...
                os.replace(f, f"{q}/{os.path.basename(f)}")
    p = f"{gdir}/rows.csv"
    if os.path.exists(p):
        d = pd.read_csv(p, low_memory=False)
        m = (
            d.apply(lambda r: contaminated(str(r.get("method")), r.get("method_stats")), axis=1)
            if "method_stats" in d
        ...
        if APPLY and m.any():
            os.makedirs(q, exist_ok=True)
            d[m].to_csv(
                f"{q}/rows_quarantined.csv",
                mode="a",
                index=False,
                header=not os.path.exists(f"{q}/rows_quarantined.csv"),
            )
            tmp = f"{p}.{os.getpid()}.tmp"
            d[~m].to_csv(tmp, index=False)
            os.replace(tmp, p)
    if bad:
        print(f"  {g:40s} {len(bad):5d} rows")
        moved += len(bad)

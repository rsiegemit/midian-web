...
sub re5k_cal "$RTE_CPU_PARTITIONS" 4 64G 2:00:00 4 routereval5k_cal
sub lrb_cal "$RTE_CPU_PARTITIONS" 4 32G 1:00:00 4 llmrouterbench_cal
sub mmlu_norep "$RTE_CPU_PARTITIONS" 4 32G 2:00:00 4 routereval_mmlu_norep_cal
sub lrb_norep "$RTE_CPU_PARTITIONS" 4 64G 6:00:00 4 llmrouterbench_norep_cal
sub re5k_norep_fast "$RTE_CPU_PARTITIONS" 4 64G 6:00:00 4 routereval5k_norep_cal \
...
for beta in 0.0 0.5; do for s in 1 2 3; do for b in 1 3 5; do   # kNN: ~4.1 h per row single-threaded; one job per (regime, seed, b), 8 threads
  OMP_NUM_THREADS=8 MKL_NUM_THREADS=8 sub re5k_norep_knn_${beta}_${s}_b$b "$RTE_CPU_PARTITIONS" 8 48G 12:00:00 1 routereval5k_norep_cal --methods knn_router --only beta=$beta,b=$b --seeds $s; done; done; done
for d in specialist heavy_tail bimodal; do for beta in 0.0 0.5; do for s in 1-15 16-30; do
  sub rep_${d}_${beta}_$s "$RTE_CPU_PARTITIONS" 4 128G 12:00:00 4 replay_1e6_split_cal --only dist=$d,beta=$beta,liar_select=low_skill_first --seeds $s; done; done; done
for beta in 0.0 0.5; do for lo in $(seq 1 10 91); do
  sub bern_${beta}_s$lo "$RTE_CPU_PARTITIONS" 4 256G 12:00:00 4 bernoulli_1e7_cal --only dist=specialist,beta=$beta,liar_select=low_skill_first --seeds $lo-$((lo+9)); done; done

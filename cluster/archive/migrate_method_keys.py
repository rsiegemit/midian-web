...
def migrate(d, apply):
    ...
    if os.path.exists(f"{d}/{keys.SENTINEL}"):
        return f"{tag}: already migrated"
    ...
    if os.path.exists(csv):
        ...
        if "method" in df:
            hit = [
                i
                for i, (m, p) in enumerate(zip(df.method, df.get("params", pd.Series(["{}"] * len(df)))))
                if touched(m, p)
            ]
            if hit:
                ...
                for i, (_, r) in zip(hit, typed.iterrows()):
                    r = r.to_dict()
                    r["params"] = r.get("params") if isinstance(r.get("params"), str) else "{}"
                    if "rid" in r and isinstance(r["rid"], str) and rid_of_row(r) != r["rid"]:
                        bad.append(("csv", r["rid"]))
                    n = keys.to_new(r["method"], load(r["params"]))
                    if n is None:
                        drop.append(i)
                        continue
                    if (n[0], jkey(n[1])) == (r["method"], jkey(load(r["params"]))):
                        continue
                    r2 = {**r, "method": n[0], "params": jkey(n[1])}
                    rid = rid_of_row(r2)
                    if rid in new_rids:
                        bad.append(("collision", rid))
                        continue
                    new_rids[rid] = ("csv", i)
                    rows_new[i] = (n[0], jkey(n[1]), rid)
    ...
    for f in sorted(glob.glob(f"{d}/rows.d/*.json")):
        r = json.load(open(f))
        if not touched(r.get("method"), r.get("params")):
            continue
        rid0 = os.path.basename(f)[:-5]
        if rid_of_row(r) != rid0:
            bad.append(("file", rid0))
        n = keys.to_new(r["method"], load(r.get("params", "{}")))
        if n is None:
            plan.append((f, None, None))
            continue
        if (n[0], jkey(n[1])) == (r["method"], jkey(load(r.get("params", "{}")))):
            continue
        pstr = jkey(n[1]) if isinstance(r.get("params"), str) or "params" not in r else n[1]
        r2 = {**r, "method": n[0], "params": pstr}
        rid = rid_of_row(r2)
        if rid in new_rids and new_rids[rid] != ("file", rid0) and new_rids[rid][0] == "file":
            bad.append(("collision", rid))
        new_rids.setdefault(rid, ("file", rid0))
        plan.append((f, r2, rid))
    rep["files_changed"] = sum(1 for _, r2, _ in plan if r2 is not None)
    rep["files_withdrawn"] = sum(1 for _, r2, _ in plan if r2 is None)
    if bad:
        kinds = pd.Series([k for k, _ in bad]).value_counts().to_dict()
        return f"{tag}: REFUSED, nothing changed: {kinds} (e.g. {bad[:3]})"
    if not apply:
        return f"{tag}: dry run {rep}"
    # ---- apply: back up originals, then rewrite
    bk = f"{d}/_premigration_v2"
    os.makedirs(f"{bk}/rows.d", exist_ok=True)
    if df is not None and (rep["csv_changed"] or rep["csv_withdrawn"]):
        with gzip.open(f"{bk}/rows_changed.csv.gz", "wt") as fh:
            df.iloc[changed].to_csv(fh, index=False)
        for i, (m, p, rid) in rows_new.items():
            df.iat[i, df.columns.get_loc("method")] = m
            df.iat[i, df.columns.get_loc("params")] = p
            if "rid" in df:
                df.iat[i, df.columns.get_loc("rid")] = rid
    ...
    for f, r2, rid in plan:
        ...
        if r2 is not None:
            ...
            json.dump(r2, open(tmp, "w"), default=str)
            os.replace(tmp, f"{d}/rows.d/{rid}.json")
        if r2 is None or os.path.basename(f) != f"{rid}.json":
            os.remove(f)

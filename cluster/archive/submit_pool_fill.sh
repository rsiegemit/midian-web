...
sub re5k "$RTE_CPU_PARTITIONS" 4 64G 4:00:00 4 pool_fill_routereval5k
sub lrb "$RTE_CPU_PARTITIONS" 4 32G 2:00:00 4 pool_fill_llmrouterbench
for beta in 0.0 0.5; do sub bern_$beta "$RTE_CPU_PARTITIONS" 4 256G 12:00:00 4 pool_fill_bernoulli_1e7 --only dist=specialist,beta=$beta,liar_select=low_skill_first; done
for d in specialist heavy_tail bimodal; do for beta in 0.0 0.5; do sub rep_${d}_$beta "$RTE_CPU_PARTITIONS" 4 128G 8:00:00 4 pool_fill_replay_1e6 --only dist=$d,beta=$beta,liar_select=low_skill_first; done; done

...
}  # what A / B draw (condensed_figs ARMS + POOLS); E-H: the fw_* arms
MIDIAN_DRAWN = lambda p: ("audit" not in p and "verify" not in p) or (
    p.get("audit") is False and p.get("verify") is False and p.get("r", 10) == 10
)  # MIDIAN; w/o defenses at r = 10
DRAWN = lambda name, params: name.startswith("fw_") or (
    name in FIG_ARMS
    and (name != "midian" or MIDIAN_DRAWN(params))
    and (name != "flat_probe_argmax" or params.get("online"))
)
FOCUS = (
    lambda c: (c["beta"] == 0 or (c["beta"] == 0.5 and c["liar_select"] == "low_skill_first"))
    and (c["backend"] != "llm" or c["dist"] == "specialist")
    and (c["backend"] != "routereval" or c["dist"] in ("strong_to_weak", "all"))
)
...
def figure_grids(cfg):
    ...
    ab = {x for x in g if re.match(r"(va_b|rivals_b|pool_fill|tuned_wsb)_", x)} | {
        "routereval_mmlu5k",
        "llmrouterbench_pool",
        "fw_live_n100",
        "learned_n100",
        ...
        "learned_n10k",
        "live_n10k_cartel_random",
    }
    re_ = {x for x in g if x.startswith(("fw_routereval_", "re_sl_"))}
    return sorted(
        x for x in (live | ab | re_) & g if "shuffled" not in x
    )  # shuffled MIDIAN cohort: removed from the figures


def have(grid):
    d = f"{R}/{grid}"
    s = {f[:-5] for f in os.listdir(f"{d}/rows.d")} if os.path.isdir(f"{d}/rows.d") else set()
    if os.path.exists(f"{d}/rows.csv"):
        try:
            s |= set(pd.read_csv(f"{d}/rows.csv", usecols=["rid"]).rid.dropna())
...
def pooled_elsewhere(cfg, grid, _c={}):
    ...
    if "k" not in _c:
        _c["k"] = {
            (sp["name"], blk["backend"], c["n"])
            for g in cfg["grids"]
            if g.startswith("pool_fill_")
...
def main():
    ...
    packed = (
        {l.split(None, 1)[1].strip() for l in open(f"{L}/focus_packed.txt")}
        if os.path.exists(f"{L}/focus_packed.txt")
        else set()
    )
    for line in open(f"{L}/focus_pack_plan.tsv"):
        g, m, only, s = line.rstrip("\n").split("\t")
        if f"{g}|{m}|{only}|{s}" in packed:
            continue  # handed to a pack: counted via inflight if its job is alive
        for mm in m.split(","):
            held[(g, mm, str(s))].append(dict(kv.split("=") for kv in only.split(",") if kv))
    ...
    per_method = {
        tuple(n[len("rte_") :].split("__", 1)) for s, n in q.values() if n.startswith("rte_") and "__" in n
    }  # rte_<grid>__<method> jobs
            ...
            for cell in cells(blk):
                ...
                for seed in seeds(blk["seeds"]):
                    for sp in specs:
                        ...
                        if any(
                            row_id({**cell, "liar_select": ls}, sp["name"], sp["params"], seed) in done for ls in alt
                        ):
                            t[1] += 1
                            continue  # beta 0: the tag is inert
                        if (sp["name"], cell["backend"], cell["n"]) in pooled_elsewhere(cfg, grid):
                            t[2] += 1
                            continue
                        if (
                            grid in whole
                            or (grid, sp["name"]) in per_method
                            or any(covers(f, cell) for f in held.get((grid, sp["name"], str(seed)), []))
                        ):
                            t[2] += 1
                            continue
    ...
    for g, (e, d, q) in sorted(tot.items()):
        if e - d - q:
            print(f"{g:42s} {e:8d} {d:6d} {q:6d} {e - d - q:5d}")
    print(f"TOTAL gap rows: {sum(gaps.values())} in {len(gaps)} units")
    if a.plan_out:
        with open(a.plan_out, "w") as f:
            for g, m, only, s in sorted(gaps):
                f.write(f"{g}\t{m}\t{only}\t{s}\n")

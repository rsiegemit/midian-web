...
ap.add_argument("--since", default="2026-09-22T14:45")
a = ap.parse_args()
raw = subprocess.run(
    ["sacct", "-S", a.since, "-E", "now", "--format=JobID,JobName%80,State,ElapsedRaw,MaxRSS", "-n", "-P", "--units=M"],
    capture_output=True,
    text=True,
).stdout
...
bat = d[d.id.str.endswith(".batch")].copy()
bat["rssM"] = pd.to_numeric(bat.rss.str.rstrip("M"), errors="coerce")
j = top.merge(bat[["base", "rssM"]], on="base", how="left")
j["sec"] = pd.to_numeric(j.sec, errors="coerce")
j = j[j.state.eq("COMPLETED") & j.name.str.startswith("rte_fw_live")]
j["size"] = j.name.str.extract(r"fw_live_(n\d+k?)")[0]
j["method"] = j.name.str.split("__").str[-1]
g = j.groupby(["size", "method"]).agg(
    n=("sec", "size"),
    p50=("sec", lambda s: s.quantile(0.5) / 60),
    p95=("sec", lambda s: s.quantile(0.95) / 60),
    mx=("sec", lambda s: s.max() / 60),
    rss95=("rssM", lambda s: s.quantile(0.95)),
)
g = g[g.n >= 10].round(0)
...
if len(g) < 10:
    print(
        f"only {len(g)} (size, framework) cells with >= 10 completed jobs since {a.since}; keeping the existing table"
    )
else:
    ...
    g.to_csv(out)
    print(f"wrote {out}: {len(g)} cells from {len(j)} completed jobs since {a.since}")

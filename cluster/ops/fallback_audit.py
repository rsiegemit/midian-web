...
for g in sorted(os.listdir(R)):
    if not (g.startswith("fw_") or g.startswith("live_") or g.startswith("learned_")): continue
    ...
    p = f"{R}/{g}/rows.csv"
    if os.path.exists(p):
        try: recs += pd.read_csv(p, low_memory=False).to_dict("records")
        except Exception: pass
    seen = set()
    for r in recs:
        rid = r.get("rid")
        ...
        seen.add(rid)
        m = str(r.get("method", ""))
        if not (m.startswith("fw_") or m == "llm_supervisor"): continue
        ms = r.get("method_stats")
        if isinstance(ms, str):
            try: ms = json.loads(ms)
            except Exception: ms = None
        if not isinstance(ms, dict): continue
        picks = ms.get("picks", 0); fb = ms.get("fallbacks", 0); fl = ms.get("failures", 0); bad = ms.get("bad_name", 0)
        tot = picks + fb + fl + bad
        if tot == 0: continue
        rows.append(dict(grid=g, method=m, fallback=fb / tot, failure=fl / tot, nonpick=(fb + fl + bad) / tot,
                         errors=ms.get("errors", ms.get("bridge_errors", 0)), success=r.get("success")))
...
print(df.nonpick.describe(percentiles=[.5, .9, .95, .99]).round(3).to_string())
...
print("\nper-framework mean non-pick rate (behaviour, not failure -- e.g. a framework that answers instead of delegating):")

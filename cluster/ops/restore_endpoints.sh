...
# Usage: cluster/ops/restore_endpoints.sh <snapshot_dir> [fleet_jobid_for_unaliased_keys]
set -uo pipefail
...
for f in "$1"/*.json; do
  ...
  [ -n "$job" ] && squeue -h -j "$job" -o %T 2>/dev/null | grep -q RUNNING && "$PY" "$RTE_REPO/cluster/slurm/_register_endpoint.py" add "$key" "$url" && echo "restored $key"
done

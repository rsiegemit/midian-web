...
def queue():
    out = subprocess.run(
        ["squeue", "-u", os.environ["USER"], "-h", "-o", "%i %T %j"], capture_output=True, text=True, check=True
    ).stdout
...
def inflight(q=None):
    ...
    for f in glob.glob(f"{L}/launch_*.txt") + glob.glob(f"{L}/resubmit_*.txt") + glob.glob(f"{L}/*_packed.txt"):
        ...
        grid = base[len("launch_") : -4] if base.startswith("launch_") else None
        for line in open(f):
            p = line.split(None, 1)
            if len(p) != 2 or p[0] not in q:
                continue
            f4 = p[1].strip().split("|")
            if grid and len(f4) == 3:
                key = (grid, *f4)
            elif len(f4) == 4:
                key = tuple(f4)
            else:
                continue  # job-level submitters: see by job name
...
if __name__ == "__main__":
    ...
    for k, v in sorted(dup.items())[:50]:
        print("  DUP", "|".join(map(str, k)), v)
    ...
    for (n, s), c in sorted(other.items(), key=lambda x: -x[1])[:40]:
        print(f"  {c:5d} {s:9s} {n}")

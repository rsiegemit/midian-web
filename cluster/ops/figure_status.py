...
def ab(name):
    ...
    want = {
        ...
        for a in arms
        for r in regs
        for b in (("-",) if a in budgetless else ("1", "3", "5"))
    }
...
def shortlist(name, srcs=None):
    d = pd.read_csv(f"{OUT}/{name}.csv")
    k = "frameworks" if "frameworks" in d else "k"
    ...
    if "n" in d and srcs:
        ...
        miss = []
    print(
        f"{name}: {len(d)} bars, {len(miss)} missing slots, {int(d.star.sum())} rerun-outstanding, "
        f"{int((d[k] < MIN_FW).sum())} under MIN_FW"
    )
    for m in miss:
        print("   MISSING", m)
...
def grids():
    for g in GRIDS + H30_FW + sorted({x for v in H30_REF.values() for x in v}):
        ...
        if g.startswith(("fw_", "re_sl_")):  # framework grids: rows() drops fw_ arms, so read them directly
            ...
            df = fwrows(g)
            df = df if df.empty else df[df.method.astype(str).str.startswith("fw_") | (df.method == "oracle")]
        else:
            df = pd.concat([t.rows(g, n) for n in {x[0] for x in w}], ignore_index=True)
        have = (
            set()
            if df.empty
            else {
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out", help="the figures' directory (default: $RTE_FIG_OUT or figures/paper)")
    OUT = fig_out(ap.parse_args().out)
    for f in ("A_live_allb", "B_families_allb"):
        ab(f)
    shortlist("E_shortlists_by_n", MAIN)
    shortlist("F_shortlists_1e5")
    shortlist("G_shortlist_lift_1e5")
    shortlist(
        "H_routereval_shortlists", ["tfidf", "embed", "dense", "sota", "declared", "va_cohort"]
    )  # body: best instruction variant per family
    print("switched to erratum-30 rows:", sorted(t.switched()))
    print("grids planned vs landed:")
    grids()

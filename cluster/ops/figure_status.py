"""What the figures still lack, read from their own CSVs, plus planned-vs-landed rows of the grids still running.
    RTE_DATA=... python scripts/ops/figure_status.py
...
(star), fewer than MIN_FW frameworks. Grids: set comparison of planned vs landed (seed_tables.planned / rows)."""
...
from shortlist_condensed import MAIN, MIN_FW

OUT = f"{ROOT}/figures/condensed_sample"
...
def grids():
    from shortlist_figs import H30_FW, H30_REF
    for g in GRIDS + H30_FW + sorted({x for v in H30_REF.values() for x in v}):
        ...
        if g.startswith(("fw_", "re_sl_")):                     # framework grids: rows() drops fw_ arms, so read them directly
            from shortlist_figs import rows as fwrows

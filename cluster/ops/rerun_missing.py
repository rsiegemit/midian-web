...
(method, dist, beta, liar_select, seed) exactly as cluster/slurm/launch_units.sh would. Refuses a grid that still has jobs queued
...
ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.realpath(__file__)))); sys.path.insert(0, ROOT)
...
for g in args.grids:
    ...
    for m, d, be, ls, s in units:
        ...
        cmd = ["sbatch", "--parsable", "-p", PART, "-A", ACCT, "-c", "1", "--mem=40G",
               ...
               "cluster/slurm/run_grid.sbatch", g, "--methods", m, "--only", only, "--seeds", str(s)]
        ...
        if os.environ.get("RTE_WIDE_PARTITIONS"):         # widen once queued (optional, cluster/cluster.env.example)
            subprocess.run(["scontrol", "update", f"JobId={j}", f"Partition={os.environ['RTE_WIDE_PARTITIONS']}"], capture_output=True)

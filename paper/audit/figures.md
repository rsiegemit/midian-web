...
- Effect: A_live_allb.csv "n = 10,000, cartel, best learned router, b=1" reads "INCOMPLETE POOL, missing knn_router, knn_router_online" and is
...
- Numeric impact, recomputed with a loader that keys rows.d by file name (all other A/B pool bars match to 4 d.p.):
  A live 10^4 cartel b=1 best learned router: plotted 0.5344 (flat_nsw x2, disrouter x1), correct 0.5156 (flat_nsw x2, knn_online x1): bar too HIGH by 0.019.
...
- Direction: biases the pooled rival DOWN where a strong candidate is partially landed (favours MIDIAN-VA). Figure B (and A whenever a pool grid is partial).
...
### F4. [HIGH] C: the excluded constant-cost learned router (flat_nsw, 50 comparisons/query) is CHEAPER than MIDIAN at every n and than MIDIAN-VA from 10^5 up, and it is the majority pick in 10 A/B bars -- the legend/code say "2 of 29"
- Evidence: `scripts/efficiency_figs.py:79-80` drops any pool member whose fitted slope is "constant" from the learned band. cost_by_n.csv:
  flat_nsw_router work = 50 at every n (0 messages + 50 comparisons); MIDIAN = 46, 69, 92, 115, 138, 161 and MIDIAN-VA = 25, 36, 47, 58, 69, 80 at n = 10^2..10^7.
  C_routing_work_vs_n.csv draws "best learned router (∝ n^0.97-0.99)" from cluster_head (24.6 -> 1.0e6) to disrouter (54 -> 3.7e6).
...
- Reading the figure as drawn: title "MIDIAN grows like log n", the learned band grows ~n, so MIDIAN looks orders of magnitude cheaper than the best learned router.
  For the member A/B actually pick in 10 bars, the true per-query work is 50: below MIDIAN (69-161) for n >= 10^3 and below MIDIAN-VA (58-80) for n >= 10^5.
  There is no on-figure mark that a member was excluded.
- Direction: favours MIDIAN / MIDIAN-VA on the routing-work claim (C is the only figure making the "advantage over flat methods" point, guide #21).
...
- With 3 seeds (live 10^4, live 10^5, RouterEval 5,000 -- A's two right-hand groups and B's live and RouterEval groups) the 2.5/97.5 percentiles are
  exactly the smallest and largest seed: A live 10^5 honest b=3 MIDIAN-VA seeds 0.810/0.840/0.857 -> bar CI [0.810, 0.857]; the t-interval is
  [0.777, 0.894]. MIDIAN [0.713, 0.790] vs t [0.657, 0.848]; flat [0.703, 0.807] vs t [0.618, 0.880].
...
- Direction: overstates precision for the 3-seed groups (makes MIDIAN-VA's lead at 10^4/10^5 look more certain than it is). Figures A, B, E-H.
...
- Cases now: A/B live 10^5 best learned router honest (b1 0.613, b3 0.741, b5 0.613) and cartel (0.588, 0.741, 0.588) -- b=5 there is an
  ...
  (0.966, 0.972, 0.946 -- the F2 artefact) likewise hidden. Other non-monotone arms (LLMRouterBench cartel MIDIAN 0.866/0.835/0.850, learned
...
- `condensed_figs.py:121` ylim (0.2, 0.95 / 1.05); `shortlist_condensed.py:97` ylim (0.2, 0.95). Bar length is then value - 0.2, so ratios are distorted:
  A n=10^3 honest b=3 MIDIAN-VA 0.813 vs random 0.432 is a 1.9x ratio drawn as 2.6x; B replay random (0.242 of oracle) is drawn as a 0.04 sliver.
...
- Direction: exaggerates the gap between the top arms and the floor arms (random, declared under cartel, TF-IDF), i.e. favours whatever is tallest (MIDIAN-VA in A/B).
...
### F9. [CRITICAL] Dropping the pre-registered warm_start_bandit (n0 = 5) from the "best bandit" pool flips MIDIAN-VA vs best bandit at honest b = 5 in 3 of 5 B families
...
- Recomputed (same cross-fit, same per-seed tables, seed_tables spec) with n0 = 5 added back vs the plotted pool vs MIDIAN-VA (raw success):
    | cell | b | plotted best bandit | with n0=5 | MIDIAN-VA |
...
- Headline effect: as plotted, MIDIAN-VA beats best bandit at honest b = 5 in bernoulli (0.826 vs 0.822), replay (0.777 vs 0.760) and RouterEval
  ...
  MIDIAN-VA at every b by 0.07-0.22. At live 10^4 cartel b=1 the plotted bandit (0.588) is below VA (0.670); restored it is above (0.709).
...
- Where n0 = 0.5 was tuned is not recorded in condensed_figs; it helps on live 10^3 (x10 picks) and hurts on the B families. "Tuned" + "drop the
  pre-registered one" is a post-hoc pool change that favours MIDIAN-VA.
- Direction: biases best bandit DOWN (favours MIDIAN-VA), up to 0.11 raw (RouterEval honest b=3: 0.709 -> 0.822, i.e. 0.786 -> 0.911 of oracle in B).
...
### F10. [HIGH] The TEMPORARY HIDE_HALVING switch removes peer-reported sequential halving, which beats MIDIAN-VA in every honest b=3 cell of A and in 3 of the 4 other B families (all seeds)
- Evidence: `extra_figs.py:120,128` HIDE_HALVING = True drops every label containing "halving" (the peer-reported arm too, not only the withdrawn
  trusted-observer arm of erratum 26). Paired per seed, raw rows, b = 3 (only budget the arm ran at, plus b = 1 on bernoulli/replay):
    | cell | halving (peer) | MIDIAN-VA | diff | seeds halving wins |
...
- Effect on headlines: with the arm drawn, A's honest bars would show a rival above MIDIAN-VA at every n (and B at bernoulli, replay, RouterEval),
...
- Direction: favours MIDIAN-VA in the honest regime. Figures A, B (and C: halving's routing work is 1 comparison/task per energy.py, below MIDIAN).
- Other undrawn arms that beat MIDIAN-VA at honest b=3 (figures/bars/*.csv, 13:41): cnp_self_bid, verify_on_claim (bernoulli, replay; e.g. 10^7:
  0.838 / 0.842 vs 0.801), midian_a (RouterEval, LLMRouterBench, +0.01). None exceeds the drawn declared argmax / best learned router there, so they
...
- Legend order (`extra_figs._ranked`) ranks each entry by the ONE bar that carries its label, which is the first group's honest b=3 bar. In A that is
  n=10^2, where flat probe argmax (0.781) ranks above MIDIAN (0.775), although MIDIAN is higher at 10^4. In B it is live 10^5. In E and H it is the
...
- Colours mean different things in different figures. In A/B, #9467bd purple is "best bandit"; in E/F/H the same hex is "Qwen3 dense, I-comp".
  Blues (#3498db flat probe vs #1f77b4 MiniLM) and reds (#c0392b MIDIAN vs #d62728 / #8b0000 rerank) are close.
...
- beta=0 liar_select tags: bar_figs uses the "random"-tagged rows only (`bar_figs.py:80-81`), while seed_tables uses both tags. This matters only
  for arms that have both tags. The largest single-arm difference is 0.0006 (MIDIAN live 10^2 0.7747 vs 0.7741), so it is immaterial.
...
- A: all 5 single arms x 4 n x 2 regimes at b=3, plus every b=1/5 bar from va_b_* and rivals_b_*. That is 108 numbers, all matching to 4 d.p.,
  including MIDIAN-VA (e.g. 10^5 honest 0.8356 / b5 0.8533; 10^3 cartel 0.8074).
...
- B: all single-arm bars in the four non-live families, both regimes and all three b, match after division by the honest b=3 oracle:
  ...
  Replay pooled bars use only seeds that have all 3 shapes (100 seeds); e.g. MIDIAN-VA replay honest b=5 0.7769 -> 0.9834 of oracle.
...
- MIDIAN-VA b=5 at live 10^2 is identical honest vs cartel seed by seed. That is not a regime mix-up: n_liars is 0 vs 50 and misroute_to_liar is
  non-zero, so VA's verification simply overrides every declaration at that budget.
...
- E: all 74 bars match at the 15:48 snapshot (F3). F is the n=10^5 subset of E. G lifts check out
  (e.g. declared honest 0.6272 - 0.3789 = 0.2483, k = 10). H's MIDIAN-VA and oracle lines at 5,000 (0.7056 and 0.9022) equal B's raw values.

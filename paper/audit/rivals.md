...
Severity: CRITICAL / HIGH / MEDIUM / LOW. Bias direction relative to MIDIAN-VA.
...
- Bias: FAVOURS MIDIAN-VA. Fix: report it as broken / drop from pool, or alpha = 0 / random tie-break; it would still
  sit near flat online, below MIDIAN-VA at 1e4 b = 3 (0.679 vs VA 0.796), so the headline ranking does not flip.

### F2. The one budget-matched rival that beats MIDIAN-VA when honest is hidden from every figure (HIGH)
...
- Measured (calibrated bernoulli, b = 3, seeds 1-3):
  | cell | MIDIAN-VA | peer halving | trusted halving | oracle |
  |---|---|---|---|---|
...
- So "MIDIAN-VA beats every rival on the same probes" holds under the cartel but NOT honest, and the arm that shows this is
  off the figures. Bias: FAVOURS MIDIAN-VA in the honest panels of A/B. Recommendation: restore peer halving to A/B (or
  state in the caption that it beats VA when honest and loses under the cartel). This is a presentation choice made on
...
- Erratum 26 (CHANGES_AND_ERRATA.md:260-264) withdraws plain `sequential_halving` because "its tournament is scored by a
  ...
  (`rte/run.py:145-147`). Only MIDIAN/VA and peer halving pay the report channel.
- Consequence: either the setting grants trusted probe outcomes (then trusted halving, 0.834-0.862, is a legitimate
  budget-matched rival and beats VA honest by +0.05-0.07), or it does not (then flat argmax and the bandit/learned pools are
  given an information advantage VA lacks, which biases AGAINST VA and makes VA's wins over them conservative).
  The paper should pick one reading and say it. As drawn: the flat/bandit comparison is conservative for VA (fair to
  rivals), the omission of trusted halving is favourable to VA.

### F4. Where MIDIAN-VA beats flat probe argmax on the same probes, and why that is legitimate (FAIR; one LOW caveat)

- Measured (calibrated bernoulli, seeds 1-3):
  | cell | VA | MIDIAN-V | MIDIAN | flat frozen | flat online |
  |---|---|---|---|---|---|
  ...
  Live 1e5 b=3 honest (results/live_n100k): VA 0.81 / 0.84 / 0.86 vs flat online 0.70 / 0.74 / 0.81.
- Honest, VA == MIDIAN-V exactly, so the whole honest gain over flat is verification at promotion: VA spends b-1 probes
...
- Under the cartel the extra gain over MIDIAN-V (0.797 vs 0.742) is the audits removing lying reporters; flat is immune to
  liars (never reads reports), so VA vs flat under the cartel is again the verification gain.
- Checked no leakage path: VA never touches S; its verification probes are fresh index-seeded instances; its audits'
  `probe_at` re-runs are used only for striking reporters, not for estimates (`midian_a.py:32-37`).
- LOW caveat (already GUIDE 0.3 #6): VA spends 1.050 / 1.029 / 1.037 x n*K*b at b = 1 / 3 / 5 (measured, n = 1e3,
...
- Also note: at Q = 300 on 16 families (~19 tasks/family) "online" gives flat nothing (1e4: frozen 0.723 = online 0.723),
  so the online/frozen distinction is not what separates VA from flat on live 1e4-1e5.
...
- Why they win on bernoulli/replay honest: the honest declared channel there is `noisy_declared` = clip(S + N(0, 0.05))
  ...
  declared_argmax 0.858, cluster_head 0.831 vs oracle 0.845 and MIDIAN-VA 0.796. On live the channel is the model's own
...
- Direction: mixed. Where these arms win, they RAISE the "best learned router" bar using information VA does not read
  (conservative for VA). But the label misleads the reader about what learned routers achieve, and in bernoulli honest
...
### F6. Hyperparameter sensitivity: tuning does not flip the ranking vs MIDIAN-VA on live-like channels, with one exception (MEDIUM)

Calibrated bernoulli, n = 1e3, K = 16, b = 3, Q = 1000, seeds 1-3; VA = 0.784 honest / 0.787 cartel (beta = 0.5 lsf).
- `ucb_per_family` c (default sqrt(2), `ucb_per_family.py:11`; untuned): c = sqrt(2) 0.694, 0.3 0.723, 0.1 0.722 (same both
  regimes). +0.03 from tuning, still -0.06 below VA. Structural: t[f] starts at n*b, so the bonus c*sqrt(ln t / cnt)
...
- `warm_start_bandit` n0 (the pool winner; pre-registered 5, tuned 0.5 on unreported live seeds 11-15, `grid.yaml:1029-1041`):
  - declared noise 0.05 (the bernoulli default): honest n0 = 0.5 / 2 / 5 / 20 -> 0.809 / 0.823 / 0.821 / 0.832, all ABOVE
    VA (the prior is near-oracle, F5); cartel 0.756 / 0.769 / 0.771 / 0.765, all below VA 0.787.
  - declared noise 0.3 (closer to live's self-ratings; `DN=0.3` backend_kwargs): honest n0 = 0.1 / 0.5 / 2 / 5 ->
    0.757 / 0.787 / **0.804** / 0.793 -- n0 = 2 beats VA (0.784) by +0.02; cartel 0.741 / 0.763 / 0.772 / 0.761 vs VA 0.787.
  - So the exception: HONEST, a declared+probe Thompson bandit with a moderate prior can match or beat VA whenever the
    declared channel carries real signal; VA's margin is a cartel-robustness result. Live data agree in direction but VA
    still wins there (live 1e5 b = 3 honest: warm_start 0.68 / 0.75 / 0.78 vs VA 0.81 / 0.84 / 0.86), because live
    self-ratings are poor. n0 = 0.5 was a reasonable, properly held-out tuning; the n0 = 5 drop is harmless.
- Bias: untuned c / alpha favour VA slightly but do not change the ranking. The n0 finding means the honest-panel claim
  "VA beats the best bandit" is channel-dependent and should be phrased as such.
...
- Adapter-driven handicaps (all disclosed, all favour VA):
  1. Pre-registered hashed TF-IDF, stable sort, no dedup: at n = 1e5 the top-10 is ten clones of one agent, so every
     ...
     the framework. The dedup / MiniLM / SOTA / VA-cohort shortlists are drawn beside it, so the reader can see this.
  2. Supervisor = Qwen2.5-7B for all ten (`_common.py:20`); only Magentic-One has a 14B arm, and it is dropped from the
     ...
     best-in-list, so the ranking vs VA is bounded by retrieval, not by the supervisor.
...
- RouterEval task text "A task of family X (instance i)" (`_common.py:192`): the supervisor never sees the MMLU prompt
  (GUIDE 3.9 #3). Relative to VA this is FAIR -- VA, flat and the bandits also route per family only -- but it is weaker
  than `knn_router`, which on RouterEval embeds the real prompt (`_learned.vec` via `view.embedding`). State it.
- Advantages the adapter gives frameworks (conservative for VA): on live the numeric lie never reaches the prompt and the
...
- Budget parity: every probing rival spends exactly n*K*b (`_est.probe_successes`, `_learned.probe_set`); trueskill
  0.989-0.999x (pairs a1 = a2 dropped, GUIDE 2.7.5); VA 1.029-1.050x (F4). Probes are index-seeded, so all methods see the
...
- Cross-fitting of the pools (`seed_tables.crossfit`) avoids the max-of-noisy-means bias in the rivals' favour and against
  them alike.
- Flat argmax vs VA: VA's win on the same probes is adaptive re-allocation + audits, not leakage (F4).

## Summary (severity, direction)

| # | finding | sev. | bias |
|---|---|---|---|
| F1 | linucb_honest's UCB bonus selects lucky WEAK agents among ties; at/below random at n >= 1e4 (live 1e5: 0.37-0.42) | HIGH | favours VA (bounded by cross-fit) |
| F2 | peer-reported halving (the stated apples-to-apples control) beats VA honest (0.834 vs 0.784 at 1e3) but is hidden by HIDE_HALVING | HIGH | favours VA (honest panels) |
| F3 | "trusted observer" excludes halving but every drawn probing rival is a trusted observer | MEDIUM | mixed; must be stated |
| F4 | VA > flat on the same probes is legitimate (verification + audits); +3-5 % probes | FAIR / LOW | negligible |
| F5 | declared-only cluster_head / disrouter in "best learned router"; bernoulli declared = S + N(0, 0.05) | HIGH (label) | raises the rival bar; mislabels |
| F6 | tuning c / alpha / n0 does not flip the cartel ranking; honest, warm_start n0 = 2 beats VA when declarations carry signal | MEDIUM | honest claim is channel-dependent |
| F7 | frameworks faithful; TF-IDF clones, 7B supervisor, k = 10 are adapter handicaps (disclosed) | MEDIUM | favours VA |

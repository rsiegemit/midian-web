...
Names follow the 2026-09-24 rename (CHANGES_AND_ERRATA §8g); the quotes in the table are as audited then, with the
renamed arm in brackets.
...
|---|---|---|---|---|---|
| 1 | (c) real pools | their MLP router 0.620 / **0.607** at 1,000 real models | 0.620 / **0.620** | routereval_mmlu n=1000 | **wrong.** The MLP router reads neither declarations nor reports, so it is flat in β. 0.607 is MIDIAN's cartel value from the row above. Fixed in RESULTS.md §III.5 and RESULTS_rte_v3.md D2. |
...
| 3 | (d) shortlisted frameworks | trail MIDIAN-V [MIDIAN w/o audits] by **0.03–0.11** | **0.030–0.122** (LlamaIndex −0.122, Magentic-One −0.030) | fw_live_n1000_verified | **wrong upper end.** RESULTS.md §II.2 already says 0.03–0.12. |
| 4 | (c) cartel | VA [MIDIAN] **+0.15** over every framework | **+0.141 … +0.159** (min Magentic-One, max LlamaIndex) | fw_live_n1000_lowskill | **rounds up.** "+0.14 … +0.16" is accurate. Per-framework values in RESULTS.md §II.5 are correct. |
| 5 | (b) cost exponents | MIDIAN [w/o defenses] route **n^0.14**, flat n^1.00 | n^0.136 [0.131, 0.152] on `combined_scale`; **n^0.112 [0.106, 0.118]** on `bernoulli_scale` | both | **ambiguous provenance, not wrong.** n^0.14 is the live cross-n fit; text quoting it should name `combined_scale`. Flat is n^1.000 in both. |
...
frameworks 0.524–0.546; MIDIAN w/o defenses 0.6672 / MIDIAN 0.6837 / oracle 0.7230 at β = 0; paired framework − MIDIAN w/o defenses −0.130 …
−0.108; specialist frameworks 0.3899 vs MIDIAN w/o defenses 0.7784 (0 / 400 pairs won); n = 10k frameworks 0.257–0.367, random 0.4167,
tree 0.786–0.813; MIDIAN − KNN +0.074 / +0.070 / +0.126; MIDIAN − MLP +0.020 … +0.036; halving − MIDIAN +0.031 / +0.038 / +0.052
...
MIDIAN w/o defenses 0.6672 → 0.5686, MIDIAN w/o verification flat (0.6679 → 0.6655); 52.9 comparisons per task at 100k; 14B fallback 0.432 vs 0.604,
strict +0.116; shortlist of MIDIAN w/o audits lifts +0.030 … +0.087 with LlamaIndex +0.010 (CI covers 0); MIDIAN cohort +0.021 at β = 0.5;
20-model pool flat/MLP 0.687, MIDIAN w/o verification 0.677, MIDIAN 0.669 at β = 0 (0.664 over all β); churn MIDIAN −
MIDIAN w/o defenses −0.022 / −0.028.

## The two questions asked explicitly

**(i) "specialist 0.39 vs 0.78" is MIDIAN w/o defenses.** On the specialist cells of fw_live_n1000: frameworks 0.3899,
**MIDIAN w/o defenses 0.7784**, MIDIAN w/o audits 0.7946, MIDIAN 0.8019, oracle 0.8612. The README sentence is therefore
correct as written; if the paper wants the best arm the number is 0.80 (MIDIAN), not 0.78.
...
MIDIAN − AutoGen under the cartel, [+0.082, +0.225]) bootstrap the 30 cell × seed units directly and are therefore
...
- `paper/NUMBERS.json` regenerated on <cpu-partition> (7,700 entries). New: `v6.<source>.n<n>.<dist>.<regime>.<framework|frameworks_mean|frameworks_best|identical_cells_frac>` for the five framework shortlist sources and `v6.halving_live.n100000.<regime>.seed<k>` + `cells_done`.

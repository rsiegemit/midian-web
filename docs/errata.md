...
| 31 | 2026-09-25 | Two more supervisor behaviours failed framework units as infrastructure errors: LlamaIndex's selector returned JSON without its `choice` field, and Magentic-One's own orchestrator prompt outgrew the supervisor's 8,192-token context (deterministic). Separately, the n = 10<sup>2</sup> reference lines of the shortlist figures still pooled the pre-09-02 probe instances of `live_core_n100`, which the bars had dropped; and the runner re-read `rows.csv` with pandas' inexact float parser, so CSV-only rows drifted a few ulps per merge. | Both behaviours are invalid actions (non-picks, as in erratum 29); `live_core_n100` left out of the reference lines too; `rows.csv` read with `float_precision="round_trip"`. | Failed units rerun (four-arm ablation, text lie at 10<sup>5</sup>); n = 10<sup>2</sup> oracle / random / MIDIAN lines of Figure E recomputed from the current-instance grids; no stored value moves beyond the last-ulp drift already present. |

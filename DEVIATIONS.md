...
## Full documentation sync (2026-09-17, v6)

- Every grid folded; `paper/NUMBERS.json` regenerated (7,700 entries, 2,272 of them the new `v6.*` framework-shortlist and
  live-halving entries from `scripts/fw_variant_numbers.py`); the framework-shortlist tables in RESULTS (II.2, II.4c,
  Appendix S) are generated by `scripts/doc_tables.py` between `<!-- doc_tables:NAME -->` markers and checked by
  `--verify`; `--sync` refreshes them when the asterisked cells complete.

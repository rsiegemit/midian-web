...
| **\*** | a cell whose framework set or seed set is still incomplete (2026-09-17: Magentic-One at 10^2 with the VA cohort; the live 10^5 halving liar cells) |
...
- Pre-registered expectations are cited by id (T1–T6, V2-1…V2-11, T3-1…T3-24, T4-1…T4-6) with their verdict; misses are reported as
...
- Source documents: `RESULTS_rte.md` (phase 1), `RESULTS_rte_v2.md` (phase 2, incl. figure guide H1–H11),
  `RESULTS_rte_v3.md` (external comparisons, X1–X5), `RESULTS_rte_v4.md` (cohort modes), `RESULTS_energy.md`, `DEVIATIONS.md` (every as-run
  deviation, dated), `CHANGES_AND_ERRATA.md` (errata 1-26). Every number here is in `paper/NUMBERS.json` (`scripts/paper_numbers.py`);
  the framework-shortlist tables are generated by `scripts/doc_tables.py`, whose `--verify` checks them against this file.
...
A population of *n* LLM-backed agents (Qwen 0.5B–14B and Gemma 2B/9B behind personas) offers
services across K = 16 task families whose outcomes are programmatically checkable; n = 100 to 100,000 on the live backend,
10 to 10^7 on the calibrated synthetic one. A router must pick one agent for each task of a stream of Q tasks (300–1,000). A method sees only what it pays for: agents' **self-descriptions** (the declared
...
| **Agent frameworks (10)** | AutoGen, LangGraph, CrewAI, Magentic-One (7B and 14B supervisor), Microsoft Agent Framework, OpenAI Agents SDK, Google ADK, LlamaIndex, smolagents, CAMEL workforce — each as a supervisor choosing from a top-10 shortlist. Five shortlist sources, never pooled: the pre-registered hashed-TF-IDF top-10 over self-descriptions; the same with clones removed (`dedup`, erratum 25); MiniLM cosine over the deduped descriptions (`embed`); MIDIAN-V's probed leaf cohort; MIDIAN-VA's probed leaf cohort. Strict accounting (`success_strict`, `fallback_rate`). |
| **Self-contained rivals (18)** | flat probe argmax (frozen / online), sequential halving (peer-reported; the trusted-observer variant is never reported, erratum 26), warm-start bandit, UCB, Thompson, LinUCB-honest, declared argmax / softmax, verify-on-claim, gossip reputation, referral network, NSW router, cluster-head router, CNP bidding, route-to-k majority, cascade, LLM supervisor over the whole roster. |
...
- By shape (n = 1,000): specialist frameworks 0.390 vs MIDIAN 0.778 / VA 0.802 (0 of 400 pairs won); heavy_tail 0.630
  ...
  The heavy_tail and bimodal cells above had all ten frameworks identical: the TF-IDF top-10 was ten clones of one agent
  (erratum 25). II.2 carries the same cells rerun with four other shortlist sources; the pre-registered numbers stay as quoted.
...
- **What the shortlist does and does not fix (2026-09-14/17; erratum 25, DEVIATIONS).** The same cells, seeds and ten
  frameworks were rerun with four other shortlist sources. Frameworks pooled (best single framework), specialist; * =
  a framework still missing in that cell (Magentic-One at 10^2 with the VA cohort). Generated by `scripts/doc_tables.py`.

<!-- doc_tables:shortlist_specialist -->
| n | regime | TF-IDF (pre-registered) | dedup | MiniLM | MIDIAN-V cohort | MIDIAN-VA cohort | MIDIAN-VA itself | oracle |
|---|---|---|---|---|---|---|---|---|
| 10^2 | honest | 0.486 (0.564) | 0.499 (0.588) | 0.553 (0.643) | 0.570 (0.652) | 0.589 (0.679) | 0.782 | 0.845 |
| 10^2 | cartel | 0.467 (0.483) | 0.467 (0.479) | 0.529 (0.565) | -- | 0.568 (0.610) | 0.768 | 0.845 |
| 10^3 | honest | 0.388 (0.432) | 0.383 (0.440) | 0.540 (0.672) | 0.577 (0.672) | 0.574 (0.659) | 0.813 | 0.861 |
| 10^3 | cartel | 0.378 (0.411) | 0.382 (0.416) | 0.517 (0.591) | -- | 0.569 (0.621) | 0.807 | 0.861 |
| 10^4 | honest | 0.292 (0.367) | 0.301 (0.401) | 0.475 (0.631) | -- | 0.621 (0.747) | 0.811 | 0.859 |
| 10^4 | cartel | 0.284 (0.319) | 0.289 (0.321) | 0.437 (0.504) | -- | 0.568 (0.642) | 0.810 | 0.859 |
| 10^5 | honest | 0.379 (0.379) | 0.366 (0.561) | 0.495 (0.633) | -- | 0.593 (0.691) | 0.836 | 0.862 |
| 10^5 | cartel | 0.379 (0.379) | 0.344 (0.504) | 0.459 (0.503) | -- | 0.567 (0.667) | 0.828 | 0.862 |
<!-- /doc_tables -->

  Three things, each holding at every n from 10^2 to 10^5:
  1. **The retriever sets the framework number.** The pre-registered top-10's true skill averages 0.36 at 10^3 and 0.31
     at 10^5 against population means of 0.42 / 0.43 (TF-IDF over self-descriptions ranks Qwen2.5-0.5B's family-word-heavy
     texts first, so seven of ten frameworks sit below random at 10^4-10^5); MiniLM lifts the shortlist to 0.52 / 0.455
     and every framework with it (+0.15 at 10^3, +0.18 at 10^4, +0.12 at 10^5 on specialist); a probed cohort lifts it
     further (+0.03 to +0.15 over MiniLM). Each framework lands near its shortlist's MEAN: given ten distinct candidates the
     orchestrators choose no better than uniformly (best of the ten would give 0.74 / 0.70). `logs/diag/fw_shortlist_skill.py`.
  2. **Orchestration caps it.** Frameworks holding MIDIAN-VA's own ten probed agents, VA's pick first in the list, stay
     0.19-0.26 below VA routing to that pick, at every n and in both regimes. Declared argmax alone (0.62 / 0.58 at 10^5)
     beats every framework under every retriever.
  3. **The gap to MIDIAN-VA does not depend on the retriever.** On specialist populations VA is +0.24 over the best
     shortlist at 10^3 and +0.24 to +0.26 at 10^5, honest and cartel alike. On heavy_tail and bimodal every shortlist source
     is within 0.03 of VA itself (heavy_tail 10^3: 0.630 / 0.614 / 0.658 / 0.659 / 0.654 vs VA 0.689; bimodal 0.573 / 0.558 /
     0.558 / 0.551 / 0.549 vs VA 0.548; full tables in `results/doc_tables.md`), where description tracks skill.

  Per framework at n = 1,000 (10 seeds; honest = the liar-free cell, cartel = β = 0.5 low-skill-first):

<!-- doc_tables:per_framework_1e3 -->
| framework | TF-IDF (pre-registered), honest | TF-IDF (pre-registered), cartel | dedup, honest | dedup, cartel | MiniLM, honest | MiniLM, cartel | MIDIAN-V cohort, honest | MIDIAN-VA cohort, honest | MIDIAN-VA cohort, cartel |
|---|---|---|---|---|---|---|---|---|---|
| AutoGen | 0.380 | 0.372 | 0.376 | 0.377 | 0.471 | 0.470 | 0.623 | 0.623 | 0.621 |
| Magentic-One | 0.431 | 0.411 | 0.440 | 0.416 | 0.631 | 0.560 | 0.672 | 0.659 | 0.597 |
| LlamaIndex | 0.379 | 0.357 | 0.363 | 0.364 | 0.494 | 0.497 | 0.589 | 0.588 | 0.591 |
| smolagents | 0.376 | 0.379 | 0.384 | 0.386 | 0.466 | 0.466 | 0.562 | 0.564 | 0.581 |
| CrewAI | 0.376 | 0.380 | 0.384 | 0.385 | 0.672 | 0.591 | 0.568 | 0.572 | 0.580 |
| Google ADK | 0.432 | 0.403 | 0.431 | 0.406 | 0.672 | 0.591 | 0.608 | 0.610 | 0.580 |
| LangGraph | 0.402 | 0.370 | 0.365 | 0.399 | 0.490 | 0.492 | 0.554 | 0.569 | 0.576 |
| CAMEL | 0.362 | 0.367 | 0.354 | 0.355 | 0.490 | 0.498 | 0.559 | 0.523 | 0.533 |
| MAF | 0.367 | 0.367 | 0.370 | 0.371 | 0.525 | 0.524 | 0.523 | 0.522 | 0.523 |
| OpenAI Agents | 0.378 | 0.370 | 0.365 | 0.365 | 0.485 | 0.482 | 0.513 | 0.513 | 0.510 |
| **mean of ten** | 0.388 | 0.378 | 0.383 | 0.382 | 0.540 | 0.517 | 0.577 | 0.574 | 0.569 |
<!-- /doc_tables -->

  ADK and CrewAI are identical under every text retriever because both delegate to the first listed candidate whenever the
  orchestrator fails, which is the declared-skill argmax over the ten; Magentic-One and ADK lead under MiniLM at 10^3 (0.67).
  The dedup and MiniLM arms move the pre-registered shape means by at most 0.02 (heavy_tail at 10^2 by +0.06 / +0.10) and
  the ten frameworks now differ in every cell; no ranking against MIDIAN moves.
...
All 27 arms have rows (`knn_router[online]` after a 256 GB resubmission, 2026-09-11; peer-reported `sequential_halving`
after a second warm-up campaign, 2026-09-14/16, see below). Means over 3 seeds, 95% seed-bootstrap CI. **This is the
largest live pool in the programme; RouterEval cannot reach it** — its ceiling is the 5,000 real leaderboard LLMs.
...
**The ten-framework rows (erratum 25; II.2 for the mechanism).** The pre-registered row (0.379 for all ten in every
regime) was a clone artefact: the 10^5 population holds 3,920 distinct self-descriptions (25 copies each) and the TF-IDF
top-10 was ten copies of ONE agent in every family (16/16, 3 seeds). Per framework with the four shortlist sources
(honest = liar-free cell, cartel = β = 0.5 low-skill-first; 3 seeds):

<!-- doc_tables:per_framework_1e5 -->
| framework | TF-IDF (pre-registered), honest | TF-IDF (pre-registered), cartel | dedup, honest | dedup, cartel | MiniLM, honest | MiniLM, cartel | MIDIAN-VA cohort, honest | MIDIAN-VA cohort, cartel |
|---|---|---|---|---|---|---|---|---|
| AutoGen | 0.379 | 0.379 | 0.291 | 0.292 | 0.381 | 0.378 | 0.666 | 0.667 |
| CrewAI | 0.379 | 0.379 | 0.561 | 0.504 | 0.633 | 0.503 | 0.691 | 0.626 |
| Google ADK | 0.379 | 0.379 | 0.561 | 0.504 | 0.633 | 0.503 | 0.691 | 0.626 |
| Magentic-One | 0.379 | 0.379 | 0.446 | 0.391 | 0.601 | 0.498 | 0.660 | 0.620 |
| LlamaIndex | 0.379 | 0.379 | 0.332 | 0.309 | 0.424 | 0.424 | 0.610 | 0.593 |
| smolagents | 0.379 | 0.379 | 0.272 | 0.272 | 0.428 | 0.428 | 0.553 | 0.537 |
| LangGraph | 0.379 | 0.379 | 0.313 | 0.306 | 0.417 | 0.432 | 0.550 | 0.536 |
| MAF | 0.379 | 0.379 | 0.278 | 0.273 | 0.487 | 0.488 | 0.549 | 0.524 |
| CAMEL | 0.379 | 0.379 | 0.278 | 0.274 | 0.487 | 0.486 | 0.473 | 0.480 |
| OpenAI Agents | 0.379 | 0.379 | 0.323 | 0.313 | 0.458 | 0.454 | 0.484 | 0.459 |
| **mean of ten** | 0.379 | 0.379 | 0.366 | 0.344 | 0.495 | 0.459 | 0.593 | 0.567 |
<!-- /doc_tables -->

Against the best framework under the best shortlist, MIDIAN-VA is +0.15 honest and +0.16 under the cartel; against the
mean framework +0.24 / +0.26; at 51 comparisons and 7 messages per task against a supervisor call. The two β = 0 cells
(random and low-skill liar sets are the same world when nobody lies) differ by up to 0.03 for Magentic-One and ≤ 0.01
for the others: that is the frameworks' run-to-run noise, and the liar-free numbers above are the random cell alone.

**Peer-reported sequential halving at 10^5 (second campaign, 2026-09-14/16).** Its adaptive probe stream is not
index-seeded, so it needs live generation: three 48 h warm-ups with `RTE_CONCURRENCY` honoured (the first campaign had
silently capped it at 64) built the honest cells in 19.5 h, 28.7 h and 6.7 h (the last with a replica fleet); the liar
cells reuse those generations through the memo and add what the changed survivors need. Per cell (* = fewer than 3 seeds
landed; the missing cells are running with 3-day limits after the first follow-ons hit 24 h):

<!-- doc_tables:halving_live -->
| cell | seeds | halving per seed | oracle per seed | routes to a liar |
|---|---|---|---|---|
...
1. **Honest regime at scale.** Peer-reported halving (0.72 / 0.86 / 0.88 at n = 1k / 10k / 5k-real; the oracle's number at live 10^5, II.4c) and, on real pools,
...
| (all) | do-not-add list applied to every figure (`extra_figs.excluded`): MIDIAN with r ≠ 10, MIDIAN-SH, MIDIAN-SHA, trusted-observer halving | 2026-09-17 |
...
its control (`bernoulli_b_probe`), the b = 20 / 30 rungs, the framework shortlist reruns (`*_dd` deduplicated, `*_em` MiniLM,
`*_verified_va*` VA-cohort at 10^2-10^5; erratum 25, never pooled with the pre-registered rows) and the second live
10^5 peer-halving campaign.

Scripts: `rte.analyze`, `scripts/paper_numbers.py` (+ `fw_variant_numbers.py`), `scripts/doc_tables.py`, `scripts/dedup_compare.py`,
`scripts/scale_matrix.py`, `scripts/extra_figs.py`, `scripts/v3_figs.py`, `scripts/energy.py`, `scripts/routerbench_terms.py`,
...
## Appendix S — framework shortlist tables (generated; `python scripts/doc_tables.py`, verified with `--verify`)

Frameworks pooled (best single framework); honest = the liar-free cell (random liar set), cartel = β = 0.5 low-skill-first;
* = a framework still missing in that cell. Sources: pre-registered hashed TF-IDF; dedup (one agent per distinct
description); MiniLM cosine over the deduped descriptions; MIDIAN-V's leaf cohort; MIDIAN-VA's leaf cohort.


### S1 specialist, by shortlist source
<!-- doc_tables:shortlist_specialist -->
| n | regime | TF-IDF (pre-registered) | dedup | MiniLM | MIDIAN-V cohort | MIDIAN-VA cohort | MIDIAN-VA itself | oracle |
|---|---|---|---|---|---|---|---|---|
| 10^2 | honest | 0.486 (0.564) | 0.499 (0.588) | 0.553 (0.643) | 0.570 (0.652) | 0.589 (0.679) | 0.782 | 0.845 |
...
### S2 heavy_tail, by shortlist source
<!-- doc_tables:shortlist_heavy_tail -->
| n | regime | TF-IDF (pre-registered) | dedup | MiniLM | MIDIAN-V cohort | MIDIAN-VA cohort | MIDIAN-VA itself | oracle |
|---|---|---|---|---|---|---|---|---|
| 10^2 | honest | 0.557 (0.572) | 0.614 (0.649) | 0.657 (0.675) | 0.640 (0.670) | 0.647 (0.676) | 0.654 | 0.729 |
| 10^2 | cartel | 0.558 (0.568) | 0.604 (0.627) | 0.656 (0.675) | -- | 0.630 (0.660) | 0.656 | 0.729 |
| 10^3 | honest | 0.630 (0.630) | 0.614 (0.638) | 0.658 (0.677) | 0.659 (0.709) | 0.654 (0.704) | 0.689 | 0.734 |
| 10^3 | cartel | 0.630 (0.630) | 0.614 (0.636) | 0.658 (0.677) | -- | 0.647 (0.681) | 0.686 | 0.734 |
<!-- /doc_tables -->

### S3 bimodal, by shortlist source
<!-- doc_tables:shortlist_bimodal -->
| n | regime | TF-IDF (pre-registered) | dedup | MiniLM | MIDIAN-V cohort | MIDIAN-VA cohort | MIDIAN-VA itself | oracle |
|---|---|---|---|---|---|---|---|---|
| 10^2 | honest | 0.561 (0.561) | 0.557 (0.573) | 0.557 (0.573) | 0.552 (0.570) | 0.553 (0.571) | 0.549 | 0.573 |
| 10^2 | cartel | 0.561 (0.561) | 0.554 (0.566) | 0.545 (0.570) | -- | 0.539 (0.570) | 0.548 | 0.573 |
| 10^3 | honest | 0.573 (0.573) | 0.558 (0.573) | 0.558 (0.573) | 0.551 (0.573) | 0.549 (0.572) | 0.548 | 0.573 |
| 10^3 | cartel | 0.573 (0.573) | 0.557 (0.572) | 0.545 (0.572) | -- | 0.539 (0.567) | 0.545 | 0.573 |
<!-- /doc_tables -->

### S4 per framework, n = 100,000
<!-- doc_tables:per_framework_1e5 -->
| framework | TF-IDF (pre-registered), honest | TF-IDF (pre-registered), cartel | dedup, honest | dedup, cartel | MiniLM, honest | MiniLM, cartel | MIDIAN-VA cohort, honest | MIDIAN-VA cohort, cartel |
|---|---|---|---|---|---|---|---|---|
| AutoGen | 0.379 | 0.379 | 0.291 | 0.292 | 0.381 | 0.378 | 0.666 | 0.667 |
...
### S6 per framework, n = 100
<!-- doc_tables:per_framework_1e2 -->
| framework | TF-IDF (pre-registered), honest | TF-IDF (pre-registered), cartel | dedup, honest | dedup, cartel | MiniLM, honest | MiniLM, cartel | MIDIAN-V cohort, honest | MIDIAN-VA cohort, honest | MIDIAN-VA cohort, cartel |
|---|---|---|---|---|---|---|---|---|---|
| Magentic-One | 0.564 | 0.483 | 0.552 | 0.479 | 0.611 | 0.553 | 0.652 | 0.648 | 0.610 |
| CrewAI | 0.462 | 0.458 | 0.588 | 0.458 | 0.643 | 0.565 | 0.567 | 0.679 | 0.605 |
| Google ADK | 0.517 | 0.482 | 0.588 | 0.479 | 0.643 | 0.565 | 0.590 | 0.679 | 0.605 |
| AutoGen | 0.455 | 0.454 | 0.456 | 0.454 | 0.512 | 0.512 | 0.599 | 0.601 | 0.598 |
| LlamaIndex | 0.482 | 0.452 | 0.445 | 0.445 | 0.521 | 0.516 | 0.580 | 0.577 | 0.574 |
| smolagents | 0.462 | 0.462 | 0.466 | 0.466 | 0.500 | 0.499 | 0.550 | 0.550 | 0.553 |
| LangGraph | 0.486 | 0.472 | 0.480 | 0.478 | 0.525 | 0.518 | 0.554 | 0.554 | 0.550 |
| MAF | 0.474 | 0.473 | 0.475 | 0.475 | 0.527 | 0.527 | 0.543 | 0.543 | 0.541 |
| CAMEL | 0.474 | 0.467 | 0.472 | 0.471 | 0.522 | 0.514 | 0.552 | 0.528 | 0.530 |
| OpenAI Agents | 0.482 | 0.464 | 0.469 | 0.467 | 0.523 | 0.524 | 0.515 | 0.526 | 0.511 |
| **mean of ten** | 0.486 | 0.467 | 0.499 | 0.467 | 0.553 | 0.529 | 0.570 | 0.589 | 0.568 |
<!-- /doc_tables -->

### S7 live 10^5 peer-reported sequential halving, per cell
<!-- doc_tables:halving_live -->
| cell | seeds | halving per seed | oracle per seed | routes to a liar |
|---|---|---|---|---|
| β = 0, low-skill cartel | 3 / 3 | 0.830 / 0.860 / 0.900 | 0.830 / 0.860 / 0.897 | 0.00 / 0.00 / 0.00 |

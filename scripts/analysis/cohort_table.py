...
def delta(df, base, lab, b, beta, ls):
    ...
    if base not in w or lab not in w: return None
...
def main():
    ...
    a = ap.parse_args()
    regimes = [("beta = 0 (no liars)", 0.0, "random")] if a.regime == "beta0" else \
              [("cartel (beta = 0.5, low-skill-first)", 0.5, "low_skill_first")] if a.regime == "cartel" else \
              [("beta = 0 (no liars)", 0.0, "random"), ("cartel (beta = 0.5, low-skill-first)", 0.5, "low_skill_first")]
    frames = {}
    for name, grids in POOLS:
        try: frames[name] = load(grids)
        except SystemExit: print(f"[cohort_table] {name}: no rows, skipped", file=sys.stderr)

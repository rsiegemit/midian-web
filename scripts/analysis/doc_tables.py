...
def fw_rows(grid, kind):
    if (grid, kind) not in _cache:
        ...
        _cache[(grid, kind)] = (
            pd.DataFrame()
            if df.empty
            else select(df, kind).assign(regime=lambda d: [regime(b, l) for b, l in zip(d.beta, d.liar_select)])
        )
...
def cell(fw, dist, reg, expected=10):
    ...
    if reg == "beta0" and q.liar_select.nunique() > 1:
        q = q[q.liar_select == "random"]  # liar-free: the random cell, once
    if q.empty:
        return None
...
def ref(n, reg, dist, arm):
    ...
    if df.empty:
        return float("nan")
    q = df[(df.method == arm) & (df.params == "{}") & (df.dist == dist)].assign(
        regime=lambda d: [regime(b, l) for b, l in zip(d.beta, d.liar_select)]
    )
    q = q[q.regime == reg]
    ...
    for n in (100, 1000, 10000, 100000):
        for reg in ("beta0", "cartel"):
            ...
            if all(x == "--" for x in cells):
                continue
            lines.append(
                f"| 10^{int(np.log10(n))} | {'honest' if reg == 'beta0' else 'cartel'} | "
                + " | ".join(cells)
                + f" | {ref(n, reg, dist, 'midian'):.3f} | {ref(n, reg, dist, 'oracle'):.3f} |"
            )
    return "\n".join(lines)
    ...
    for name, kind, honest, cartel in SOURCES:
        ...
        for reg, grids in (("honest", honest), ("cartel", cartel)):
            ...
            if not g:
                continue
            fw = fw_rows(g, kind)
            q = fw[(fw.dist == dist) & (fw.regime == ("beta0" if reg == "honest" else "cartel"))]
            if reg == "honest" and q.liar_select.nunique() > 1:
            ...
            cols.append(
                (
                    f"{name}, {reg}",
                    q.groupby("method").success.mean(),
                    q.groupby("method").seed.nunique(),
                    {m for m in q.method.unique() if (g, m, dist, r) in PENDING()},
                )
            )
...
def halving_table():
    """Live 10^5 peer-reported halving, per cell: seeds landed, success per seed, oracle per seed; * while a cell is
    partial."""
    ...
    for (beta, ls), q in h.groupby(["beta", "liar_select"]):
        ...
        star = "*" if len(q) < 3 else ""
        lines.append(
            f"| β = {float(beta):g}, {'low-skill cartel' if ls == 'low_skill_first' else 'random liars'} | {len(q)} / "
            f"3{star} | "
            + " / ".join(f"{v:.3f}" for v in q.success)
            + " | "
            + " / ".join(f"{o.get((b, l, int(s)), float('nan')):.3f}" for b, l, s in zip(q.beta, q.liar_select, q.seed))
            + " | "
...
TABLES = {
    "shortlist_specialist": lambda: shortlist_table("specialist"),
    "shortlist_heavy_tail": lambda: shortlist_table("heavy_tail"),
    "shortlist_bimodal": lambda: shortlist_table("bimodal"),
    "per_framework_1e5": lambda: per_framework_table(100000),
    "per_framework_1e3": lambda: per_framework_table(1000),
    "per_framework_1e2": lambda: per_framework_table(100),
    "halving_live": halving_table,
}

...
MMLU subject named in the prompt (57; predicted at test by 5-NN over the probe prompts' embeddings). MIDIAN w/o defenses
...
def embedllm(Etr, Ytr, Ete, dim=64, epochs=5, batch=2048, seed=0):
    ...
    n_q, m = Ytr.shape
    Vm = torch.nn.Parameter(torch.randn(m, dim) * 0.1)
    W = torch.nn.Linear(Etr.shape[1], dim)
    w = torch.nn.Linear(dim, 1)
    ...
    qi, mi = np.divmod(np.arange(n_q * m), m)
    rng = np.random.default_rng(seed)
    for _ in range(epochs):
        perm = rng.permutation(n_q * m)
        for lo in range(0, len(perm), batch):
            b = perm[lo : lo + batch]
            q, mm = torch.tensor(qi[b]), torch.tensor(mi[b])
            logit = w(W(Xq[q]) * Vm[mm]).squeeze(1)
            loss = torch.nn.functional.binary_cross_entropy_with_logits(logit, Y[q, mm])
            opt.zero_grad()
            loss.backward()
            opt.step()
    with torch.no_grad():
...
def probe_family(fam_tr, fam_te, Ytr, b, rng):
    """b probes per family per candidate from train; estimate (F, m); pick argmax of the test prompt's family. Returns
    picks, labels used."""
    F = fam_tr.max() + 1
    est = np.zeros((F, Ytr.shape[1]))
    used = 0
    for k in range(F):
        idx = np.flatnonzero(fam_tr == k)
        idx = rng.choice(idx, size=min(b, len(idx)), replace=False)
        est[k] = Ytr[idx].mean(0) if len(idx) else Ytr.mean(0)
        used += len(idx) * Ytr.shape[1]
    return np.argmax(est[fam_te], axis=1), used, est


def tuned(name, cands, Yva, Yte):
    """Pick the candidate with the best VALIDATION μ (their val split), report its TEST μ. cands = [(label, pick_va,
    pick_te, labels_used)]."""
    best = max(cands, key=lambda c: score(c[1], Yva))
    return f"{name} [val-tuned: {best[0]}]", best[2], best[3]
...
def run(ds):
    ...
    for K in (3, 16, 64, 128):
        km = KMeans(n_clusters=K, random_state=SEED, n_init=10).fit(Etr)
        fams[f"cluster{K}"] = (km.labels_, km.predict(Ete))
    ...
    for m, cfgs in d["hard"].items():
        for cfg in CONFIGS:
            dd = cfgs[cfg]["data"]
            key = "train_label" if ds == "harness_truthfulqa_mc_0" else "train_score"
            Ytr, Yte, Yva = (
                np.asarray(dd[key], float),
                np.asarray(dd["test_score"], float),
                np.asarray(dd["val_score"], float),
            )
            bsm = Yte.mean(0).max()
            n_tr = Ytr.size
                ...
                rows.append(
                    {
                        ...
                        "router": name,
                        "mu": score(pick, Yte),
                        "V_B": score(pick, Yte) / bsm,
                        "V_R": score(pick, Yte) / ACC_REF[ds],
                        "labelled_outcomes": labels,
                        "bsm": bsm,
                        "oracle": float(Yte.max(1).mean()),
                    }
                )

            # --- their baselines
            nn = NearestNeighbors(n_neighbors=5, metric="cosine").fit(Etr)
            _, nb = nn.kneighbors(Ete)
            add("PRKnn (theirs)", np.argmax(Ytr[nb].mean(1), 1), n_tr)
            lab_tr, lab_te = fams["cluster3"]
            best = np.array([np.argmax(Ytr[lab_tr == c].mean(0)) if (lab_tr == c).any() else 0 for c in range(3)])
            ...
            add(
                "MLPR (theirs, sklearn)",
                np.argmax(
                    MLPRegressor(hidden_layer_sizes=(256,), max_iter=100, batch_size=32, random_state=SEED)
                    .fit(Etr, Ytr)
                    .predict(Ete),
                    1,
                ),
            ...
            add(
                "best single model", np.full(len(Yte), int(np.argmax(Ytr.mean(0)))), n_tr
            )  # best on train, applied to test
            # --- SOTA (LLMRouterBench 2026 leaders with released code): Avengers top-1 = per-cluster
            # accuracy ranking on ALL train labels
            for K in (16, 64):
                lab_tr, lab_te = fams[f"cluster{K}"]
                best = np.array(
                    [
                        np.argmax(Ytr[lab_tr == c].mean(0)) if (lab_tr == c).any() else int(np.argmax(Ytr.mean(0)))
                        for c in range(K)
                    ]
                )
            ...
            Eall = np.vstack([Eva, Ete])
            nv = len(Eva)
            c = []
            for k in (5, 20, 50):
                nb = NearestNeighbors(n_neighbors=k, metric="cosine").fit(Etr).kneighbors(Eall)[1]
                pk = np.argmax(Ytr[nb].mean(1), 1)
                c.append((f"k={k}", pk[:nv], pk[nv:], n_tr))
            ...
            for K in (3, 16, 64, 128):
                ...
                best = np.array(
                    [
                        np.argmax(Ytr[lab_tr == q].mean(0)) if (lab_tr == q).any() else int(np.argmax(Ytr.mean(0)))
                        for q in range(K)
                    ]
                )
            ...
            for a in (0.1, 1.0, 10.0):
                pr = Ridge(alpha=a).fit(Etr, Ytr).predict(Eall)
                c.append((f"alpha={a}", np.argmax(pr[:nv], 1), np.argmax(pr[nv:], 1), n_tr))
            add(*tuned("LinearR (theirs, ridge)", c, Yva, Yte))
            c = []
            for h in (256, 1024):
                pr = (
                    MLPRegressor(hidden_layer_sizes=(h,), max_iter=100, batch_size=32, random_state=SEED)
                    .fit(Etr, Ytr)
                    .predict(Eall)
                )
                c.append((f"hidden={h}", np.argmax(pr[:nv], 1), np.argmax(pr[nv:], 1), n_tr))
            add(*tuned("MLPR (theirs, sklearn)", c, Yva, Yte))
            c = []
            for dim, ep in ((64, 5), (232, 20)):
                pk = embedllm(Etr, Ytr, Eall, dim=dim, epochs=ep)
                c.append((f"dim={dim},ep={ep}", pk[:nv], pk[nv:], n_tr))
            add(*tuned("EmbedLLM MF (ICLR25)", c, Yva, Yte))
            c = []
            for fname in ("cluster3", "cluster16"):
                ftr, fte = fams[fname]
                fva = fams_va[int(fname[7:])]
                for b in B_PROBES:
                    pk_te, used, est = probe_family(ftr, fte, Ytr, b, np.random.default_rng(SEED))
                    pk_va = np.argmax(est[fva], axis=1)
                    c.append((f"{fname},b={b}", pk_va, pk_te, used))
            add(*tuned("probe table", c, Yva, Yte))
            # --- ours
            for fname, (ftr, fte) in fams.items():
                if fname == "cluster64":
                    continue  # Avengers' partition; probe tables use 3 / 16 / subject
                if fte is None:
                    fte = (
                        KNeighborsClassifier(n_neighbors=5).fit(Etr, ftr).predict(Ete)
                    )  # subject predicted from text embeddings
                for b in B_PROBES:
                    pick, used, est = probe_family(ftr, fte, Ytr, b, rng)
                    add(f"probe_{fname}_b{b}", pick, used)
                    if b == 10 and fname == "cluster16":
                        sc = est[fte]
                        assert all(midian_tree_pick(s) == int(np.argmax(s)) for s in sc[:500]), "MIDIAN tree != argmax"
            print(f"[{ds}] m={m} {cfg} done", flush=True)
    df = pd.DataFrame(rows)
    df.to_csv(f"{OUT}/{ds}.csv", index=False)
    return df
...
def summarise():
    ...
    md = [
        "# RouterEval on its own terms (hard setting; mean over the three pool configs, as their harness reports)",
        "",
    ]
    ...
    for m in sorted(df.m.unique()):
        t = (
            ...
            .agg(
                ...
                V_B=("V_B", "mean"),
                V_R=("V_R", "mean"),
                labels=("labelled_outcomes", "mean"),
                n=("mu", "size"),
            )
            .sort_values("mu", ascending=False)
        )
        md += [
            f"## m = {m} candidates ({df[df.m == m].dataset.nunique()} datasets × 3 configs)",
            "",
            "| router | μ | V_B | V_R | labelled outcomes | runs |",
            "|---|---|---|---|---|---|",
        ]
        md += [
            f"| {r} | {v.mu:.4f} | {v.V_B:.3f} | {v.V_R:.3f} | {v.labels:,.0f} | {int(v.n)} |" for r, v in t.iterrows()
        ] + [""]
    ...
    keep = [
        ...
        for c in [
            ...
            "best single model",
            "PRKnn (theirs)",
            "C-RoBERTa-cluster (theirs)",
            "LinearR (theirs, ridge)",
            "MLPR (theirs, sklearn)",
            "Avengers top-1 K=64 (AAAI26, full labels)",
            "EmbedLLM MF (ICLR25, dim64 5ep)",
            "probe_cluster16_b10",
            "probe_cluster16_b30",
            "probe_subject_b30",
            "random (theirs)",
    ...
    df["base"] = df.router.str.replace(r" \[val-tuned: .*\]$", " [val-tuned]", regex=True)
    pv2 = df[df.m == 1000].pivot_table(index="dataset", columns="base", values="mu")
    tuned_cols = [c for c in pv2.columns if "[val-tuned]" in c]
    if tuned_cols:
        md += [
            "",
            "## m = 1000, per dataset (μ), hyperparameters selected on their validation split",
            "",
            "| dataset | " + " | ".join(tuned_cols) + " |",
            "|---" * (len(tuned_cols) + 1) + "|",
        ]
        md += [
            "| "
            + ds
            + " | "
            + " | ".join("" if pd.isna(pv2.loc[ds, c]) else f"{pv2.loc[ds, c]:.3f}" for c in tuned_cols)
            + " |"
            for ds in pv2.index
        ]
    md += ["## m = 1000, per dataset (μ)", "", "| dataset | " + " | ".join(keep) + " |", "|---" * (len(keep) + 1) + "|"]
    md += [
        "| " + ds + " | " + " | ".join("" if pd.isna(piv.loc[ds, c]) else f"{piv.loc[ds, c]:.3f}" for c in keep) + " |"
        for ds in piv.index
    ]

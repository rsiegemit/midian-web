...
R = str(RTE_DATA)
B, OUT, NPZ = (
    f"{R}/data/llmrouterbench/bench-release",
    f"{R}/results/llmrouterbench_terms",
    f"{R}/data/llmrouterbench/perf_matrix.npz",
)
...
def prep():
    ...
    for d in DATASETS:
        sp = SPLIT.get(d, "test")
        tabs[d] = {f.split("/")[-2]: json.load(open(f)) for f in glob.glob(f"{B}/{d}/{sp}/*/*.json")}
    models = sorted(set.intersection(*[set(t) for t in tabs.values()]))
    assert len(models) == 20, models
    Y, fam, prompts = [], [], []
    for k, d in enumerate(DATASETS):
        recs = {m: sorted(tabs[d][m]["records"], key=lambda r: r["index"]) for m in models}
        n = min(len(v) for v in recs.values())
        Y.append(np.stack([[r["score"] for r in recs[m][:n]] for m in models], 1))
        fam += [k] * n
...
def metrics(pick, Yte, fte, rand, bsm, orc):
    ...
    return {
        "AvgAcc": acc.mean(),
        "Gain@R": (acc / rand - 1).mean(),
        "Gain@B": (acc / bsm - 1).mean(),
        "Gap@O": (1 - acc / orc).mean(),
    }


def main():
    z = np.load(NPZ, allow_pickle=True)
    Y, fam, E = z["Y"], z["fam"], z["E"]
    N, M = Y.shape
    rows = []
    for seed in SEEDS:
        ...
        per_ds = lambda pick, Yx, fx: np.array(
            [Yx[fx == k][np.arange((fx == k).sum()), pick[fx == k]].mean() for k in range(len(DATASETS))]
        )
        rand = np.array([Yte[fte == k].mean() for k in range(len(DATASETS))])
        bsm_idx = int(np.argmax(Ytr.mean(0)))
        bsm = per_ds(np.full(n_te, bsm_idx), Yte, fte)
        orc = np.array([Yte[fte == k].max(1).mean() for k in range(len(DATASETS))])

        def add(name, pick, labels):
            rows.append(
                {"seed": seed, "router": name, **metrics(pick, Yte, fte, rand, bsm, orc), "labelled_outcomes": labels}
            )

        def tuned(name, cands, labels):  # cands: (label, pick_va, pick_te)
            best = max(cands, key=lambda c: per_ds(c[1], Yva, fva).mean())
            add(f"{name} [tuned: {best[0]}]", best[2], labels)

        add("random", rng.integers(0, M, n_te), 0)
        add("best single model", np.full(n_te, bsm_idx), len(tr) * M)
        add("oracle", np.argmax(Yte, 1), 0)
        ...
        for k in (5, 20, 50):
            ...
            c.append((f"k={k}", pk[:nv], pk[nv:]))
        tuned("KNN router", c, all_lab)
        c = [
            (f"alpha={a}", *np.split(np.argmax(Ridge(alpha=a).fit(Etr, Ytr).predict(Eall), 1), [nv]))
            for a in (0.1, 1.0, 10.0)
        ]
        tuned("linear router", c, all_lab)
        c = [
            (
                f"hidden={h}",
                *np.split(
                    np.argmax(
                        MLPRegressor(hidden_layer_sizes=(h,), max_iter=100, batch_size=32, random_state=0)
                        .fit(Etr, Ytr)
                        .predict(Eall),
                        1,
                    ),
        ...
        tuned("MLP router", c, all_lab)
        c = [
            (f"dim={dim},ep={ep}", *np.split(embedllm(Etr, Ytr, Eall, dim=dim, epochs=ep), [nv]))
            for dim, ep in ((64, 5), (232, 20))
        ]
        tuned("EmbedLLM MF (ICLR25)", c, all_lab)
        c = []
        for K in (15, 64, 128):
            km = KMeans(n_clusters=K, random_state=0, n_init=10).fit(Etr)
            lab = km.labels_
            best = np.array([np.argmax(Ytr[lab == q].mean(0)) if (lab == q).any() else bsm_idx for q in range(K)])
            p = best[km.predict(Eall)]
            c.append((f"K={K}", p[:nv], p[nv:]))
        tuned("Avengers top-1 (AAAI26, full labels)", c, all_lab)
        # dataset-as-family table with the given family (their records carry task_name): best model per
        # dataset on ALL train labels
        best = np.array([np.argmax(Ytr[ftr == k].mean(0)) for k in range(len(DATASETS))])
        add("dataset table, full labels", best[fte], all_lab)
        c, cp = [], []
        for b in B_PROBES:
            probe = np.concatenate(
                [
                    rng.choice(np.flatnonzero(ftr == k), size=min(b, (ftr == k).sum()), replace=False)
                    for k in range(len(DATASETS))
                ]
            )
            est = np.stack([Ytr[probe][ftr[probe] == k].mean(0) for k in range(len(DATASETS))])
            pk = np.argmax(est, 1)
            c.append((f"b={b}", pk[fva], pk[fte]))
            fam_pred = KNeighborsClassifier(n_neighbors=5).fit(Etr[probe], ftr[probe]).predict(Eall)
            cp.append((f"b={b}", pk[fam_pred[:nv]], pk[fam_pred[nv:]]))
        tuned("probe table (family given)", c, "b×15×20")
        tuned("probe table (family predicted)", cp, "b×15×20")
        print(f"[seed {seed}] done", flush=True)
    df = pd.DataFrame(rows)
    df.to_csv(f"{OUT}/rows.csv", index=False)
    df["base"] = df.router.str.replace(r" \[tuned: .*\]$", "", regex=True)
    g = (
        df.groupby("base")
        .agg(
            AvgAcc=("AvgAcc", "mean"),
        ...
        .sort_values("AvgAcc", ascending=False)
    )
    md = [
        "# LLMRouterBench on its own terms (performance setting: 15 datasets × 20 lightweight models; 70/30 × their 5 "
        "seeds; MiniLM embeddings; hyperparameters tuned on a 20% train slice)",
        "",
        "| router | AvgAcc | sd (seeds) | Gain@R | Gain@B | Gap@O |",
        "|---|---|---|---|---|---|",
    ] + [
        f"| {r} | {v.AvgAcc:.4f} | {v.sd:.4f} | {v.GainR:+.3f} | {v.GainB:+.3f} | {v.GapO:.3f} |"

...
def regimes(df):
    for beta in sorted(df.beta.dropna().unique()):
        if np.isclose(beta, 0.0):
            yield "beta=0 (no liars)", beta, "random"
        else:
...
def main():
    ...
    for title, beta, ls in regimes(df):
        ...
        for (lab, col), g in q.groupby(["label", "col"]):
            ...
            s = (
                g.set_index(["dist", "seed"])[a.metric].dropna()
                if "dist" in g
                else g.set_index("seed")[a.metric].dropna()
            )
            ...
            st = dict(
                ...
                seeds=per_seed.size,
                units=len(s),
                std_seed=per_seed.std(ddof=1),
                var_seed=per_seed.var(ddof=1),  # spread OVER SEEDS (the CI's basis)
                std_unit=s.std(ddof=1),
                min=s.min(),
                max=s.max(),  # spread over every (shape, seed) unit
                sem=per_seed.std(ddof=1) / np.sqrt(per_seed.size),
            )
            piv[(lab, col)] = st
            out.append(
                dict(regime=title, beta=beta, liar_select=ls, label=lab, n=n, b=int(g.b.iloc[0]), metric=a.metric, **st)
            )
        ...
        print(f"\n## {title}\n")
        print(
            "cell = mean ± std over seeds [95% CI] (seeds)  — std is across per-seed means; CSV also has var, "
            "unit-level std, min, max, sem\n"
        )
        ...
        for l in labs:
            ...
            for n in ns:
                ...
                cells.append(
                    "--"
                    if v is None
                    else f"{v['mean']:.3f} ± {v['std_seed']:.3f} [{v['ci_lo']:.3f},{v['ci_hi']:.3f}] ({v['seeds']})"
                )

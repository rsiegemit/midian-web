...
def bert_win_rate(prompts):
    ...
    if os.path.exists(BERT_CACHE):
        return np.load(BERT_CACHE)
    import torch
    from transformers import AutoModelForSequenceClassification, AutoTokenizer

    model = AutoModelForSequenceClassification.from_pretrained(BERT_CKPT, num_labels=3).eval()
    tok = AutoTokenizer.from_pretrained(BERT_CKPT)
    out = np.empty(len(prompts))
    with torch.no_grad():
        for i in range(0, len(prompts), 64):
            logits = model(
                **tok(list(prompts[i : i + 64]), return_tensors="pt", padding=True, truncation=True)
            ).logits.numpy()
            p = np.exp(logits - logits.max(1, keepdims=True))
            p /= p.sum(1, keepdims=True)
            out[i : i + 64] = 1 - p[:, -2:].sum(1)
    np.save(BERT_CACHE, out)
    return out
...
def metrics(pts, acc_w, acc_s):
    ...
    auc = float(np.trapezoid(y, x / 100))
    span = x.max() / 100 - x.min() / 100
...
def main():
    ...
    for seed, (tr, te) in enumerate(
        ...
        jit = rng.normal(0, 1e-9, len(te))  # tie-break for few-valued scores
        ...
        scores = {
            "bert (RouteLLM)": bert[te],
            "random": rng.uniform(size=len(te)),
            "optimal": (ps - pw)[te] + jit,
            f"probe_family_b{B}": est[fam_pred] + jit,
            f"probe_family_b{B}_oraclefamily": est[fi[te]] + jit,
            f"knn_b{B}": KNeighborsRegressor(n_neighbors=min(KNN_K, len(probe)))
            .fit(e[probe], (ps - pw)[probe])
            .predict(e[te])
            + jit,
        ...
        for name, sc in scores.items():
            pts = curve(sc, ps[te], pw[te])
            rows.append({"seed": seed, "router": name, **metrics(pts, acc_w, acc_s)})
            curves += [{"seed": seed, "router": name, "strong_pct": x, "accuracy": y} for x, y in pts]
        rows.append({"seed": seed, "router": "weak only", "AUC": acc_w, "APGR": 0.0})
        rows.append({"seed": seed, "router": "strong only", "AUC": acc_s, "APGR": 1.0})
        print(f"[seed {seed}] done", flush=True)
    df, cv = pd.DataFrame(rows), pd.DataFrame(curves)
    df.to_csv(f"{OUT}/metrics_rows.csv", index=False)
    ...
    agg = (
        ...
        .sort_values("APGR", ascending=False)
    )
    md = [
        f"# RouteLLM's routers vs the probe-family router on RouterBench outcomes, pair {STRONG} / {WEAK} (5 "
        f"stratified splits, RouteLLM's metrics)",
        "",
    ...
    md += [
        f"| {r} | {v.APGR:.3f} | {v.APGR_sd:.3f} | {v.CPT20:.1f}% | {v.CPT50:.1f}% | {v.CPT80:.1f}% |"
        for r, v in agg.iterrows()
    ]
...
def fig(cv, agg, df):
    ...
    for r in [c for c in agg.index if c not in ("weak only", "strong only")]:
        g = cv[cv.router == r].groupby("strong_pct").accuracy.mean()
        ax.plot(
            g.index, g.values, marker=".", lw=1.6 if "probe" in r else 1.0, label=f"{r} (APGR {agg.loc[r, 'APGR']:.2f})"
        )
    w, s = df[df.router == "weak only"].AUC.mean(), df[df.router == "strong only"].AUC.mean()
    ax.axhline(w, color="grey", ls="--", label=f"{WEAK} {w:.3f}")
    ax.axhline(s, color="red", ls="--", label=f"{STRONG} {s:.3f}")
    ax.set_xlabel("Strong model calls (%)")
    ax.set_ylabel("Performance")
    ax.set_title("RouteLLM protocol on RouterBench outcomes (mean of 5 splits)")
    ax.grid(alpha=0.3)

...
SRC = {
    "fw_live_n1000_dd": "fw_live_n1000",
    "fw_live_n100_dd": "fw_live_n100",
    "fw_live_n1000_lowskill_dd": "fw_live_n1000_lowskill",
    "fw_live_n100_lowskill_dd": "fw_live_n100_lowskill",
    "fw_live_n10k_dd": "live_n10k_v2",
    "fw_live_n100k_dd": "live_n100k",
    "fw_live_n10k_cartel_dd": "fw_live_n10k_cartel",
    "fw_k_sensitivity_dd": "fw_k_sensitivity",
    "fw_appendix_dd": "fw_appendix",
    "budget_b10_fw_dd": "budget_b10_shapes",
    "churn_n1000_fw_dd": "churn_n1000",
}
SRC.update(
    {k.replace("_dd", "_em"): v for k, v in SRC.items() if k.startswith("fw_live")}
)  # embed grids pair with the same sources
...
def load(grid):
    ...
    df["key"] = (
        df.params.str.replace(r'"(dedup|retrieval)":\s*("embed"|true),?\s*', "", regex=True)
...
def regime(r):
    return (
        "beta=0"
        if float(r.beta) == 0
        else f"beta={float(r.beta):g} {'cartel' if r.liar_select == 'low_skill_first' else 'random'}"
    )
...
for g in grids:
    ...
    for d in (new, old):
        ...
        if "churn" in d:
            d["key"] = d.key + d.churn.where(
                d.churn.notna() & ~d.churn.astype(str).isin(["None", "nan", "{}"]), ""
            ).astype(str)
    ...
    if m.empty:
        print(f"\n## {g}: no paired rows yet")
        continue
    print(
        f"\n## {g} <- {SRC[g]}: {len(m)} paired rows ({new.method.nunique()} frameworks new, {old.method.nunique()} "
        f"old); "
    ...
    for (dist, reg), q in m.groupby(["dist", "regime"]):
        ...
        delta = per_seed.new - per_seed.old
        lo, hi = boot(delta) if len(delta) > 1 else (delta.mean(), delta.mean())
        cells_old = q.groupby("seed").success_old.nunique()
        cells_new = q.groupby("seed").success_new.nunique()
        print(
            f"  {dist:<11} {reg:<18} old {per_seed.old.mean():.3f} -> new {per_seed.new.mean():.3f}  delta "
            f"{delta.mean():+.3f} [{lo:+.3f},{hi:+.3f}]  "
            f"seeds {len(per_seed)}  identical-framework cells: {int((cells_old == 1).sum())}/{len(cells_old)} -> "
            f"{int((cells_new == 1).sum())}/{len(cells_new)}"
        )

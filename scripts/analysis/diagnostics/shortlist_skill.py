...
for seed in (1, 2, 3):
    ...
    def sk(sel):
        return (
            float(np.mean([S[sel[f][sel[f] >= 0], f].mean() for f in range(K)])),
            float(np.mean([S[sel[f][sel[f] >= 0], f].max() for f in range(K)])),
        )

    def topk(score, k=10):
        return {f: pool[np.argsort(-score[f][pool], kind="stable")][:k] for f in range(K)}
    ...
    B = _bm25(desc, fam)
    got["BM25 + dedup"] = topk({f: B[f] for f in range(K)})
    Eq = np.load(f"{p}/descriptions_qwen_qwen3_embedding_8b.npy")
    for tag, lbl in [
        ("", "Qwen3-8B dense (stock instr)"),
        ("_i0258be9d", "Qwen3-8B dense (instr: competent)"),
        ...
        got[lbl.replace("dense", "hybrid")] = {
            f: pool[np.argsort(-_rrf(B[f][pool], (Eq @ Fq[f])[pool]), kind="stable")][:10] for f in range(K)
        }
    ...
    for k, v in got.items():
        mu, mx = sk(v)
        rows.setdefault(k, []).append(mu)
        best.setdefault(k, []).append(mx)
    rows.setdefault("-- population mean --", []).append(float(S.mean()))
    best.setdefault("-- population mean --", []).append(float(np.mean([S[:, f].max() for f in range(K)])))

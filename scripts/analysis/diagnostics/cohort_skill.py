...
for seed in (1, 2, 3):
    ...
    w = World(
        S.shape[0], K, "specialist", 0.0, seed=seed, backend="bernoulli", backend_kwargs={"calibrate_from": Spath}
    )
    ...
    for t in w.tasks(400):
        ...
        if len(tasks) == K:
            break
    for name, mk in [
        ("MIDIAN w/o audits leaf cohort", lambda: Midian(audit=False, r=10)),
        ...
        for f, t in sorted(tasks.items()):
            ...
            sk.append(S[sel, f].mean())
            sizes.append(len(sel))
            bst.append(S[sel, f].max())
            pick.append(S[a, f])
        ...
        print(
            f"seed {seed} {name}: mean {np.mean(sk):.3f}  best-in-list {np.mean(bst):.3f}  "
            f"own pick {np.mean(pick):.3f}",
            flush=True,
        )

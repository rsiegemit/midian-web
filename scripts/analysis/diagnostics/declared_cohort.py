...
K, B, TOPK = 16, 3, 10
REG = [
    (0.0, "random", "beta=0 (honest)"),
    (0.25, "random", "beta=0.25 random"),
    (0.25, "low_skill_first", "beta=0.25 CARTEL"),
    (0.5, "random", "beta=0.5 random"),
...
for seed in (1, 2, 3):
    ...
    for beta, ls, lbl in REG:
        ...
        for f, t in sorted(tasks.items()):
            ...
            mus.append(S[sel, f].mean())
            bss.append(S[sel, f].max())
            lrs.append(w.liars[sel].mean())

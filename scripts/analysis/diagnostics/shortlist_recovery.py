...
RD = os.environ["RTE_DATA"]; K, TOPK = 16, 10
...
for n in (10000, 100000):
    for seed in (1, 2, 3):
        ...
        desc = json.load(open(f"{P}/descriptions.json")); S = np.load(f"{P}/S.npy").astype(np.float32)
        ...
        X = _hash_tfidf(desc + fdesc); Xa, Xf = X[:n], X[n:]
        lists = {"tfidf": {f: np.argsort(-(Xa @ Xf[f]), kind="stable")[:TOPK] for f in range(K)},
        ...
        for tag, sfx in TAGS.items():
            if sfx: lists["dense" + sfx] = rank(lambda f, Fq=np.load(f"{P}/families_qwen_qwen3_embedding_8b{tag}.npy"): Eq @ Fq[f])
            T = np.load(f"{P}/{SOTA}{tag}.npy"); lists["sota" + sfx] = {f: T[f][T[f] >= 0][:TOPK] for f in range(K)}
        for reg, (beta, ls) in REG.items():
            ...
            for t in w.tasks(400):
                f = int(t.family)
                if f in va: continue
                a = m.fetch(t); coh = m.leaves[m.leaf_of[a]]; va[f] = np.concatenate([[a], coh[(coh >= 0) & (coh != a)]])[:TOPK]
                if len(va) == K: break
            L["va_cohort"] = va
            for src, sl in L.items():
                if sl is None: continue
                recs.append(dict(n=n, seed=seed, regime=reg, shortlist=src,
...
r = j.groupby(["n", "regime", "shortlist"]).agg(frameworks=("arm", "nunique"), fw_mean=("mean", "mean"), rec_mean=("recovery", "mean"),
                                                rec_min=("recovery", "min"), rec_max=("recovery", "max")).reset_index()
out = d.merge(r, on=["n", "regime", "shortlist"], how="left")
os.makedirs(f"{ROOT}/paper/diagnostics", exist_ok=True)
out.to_csv(f"{ROOT}/paper/diagnostics/shortlist_recovery.csv", index=False); j.to_csv(f"{ROOT}/paper/diagnostics/shortlist_recovery_per_framework.csv", index=False)

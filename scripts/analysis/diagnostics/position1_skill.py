...
for seed in (1, 2, 3):
    ...
    def rank(score): return {f: pool[np.argsort(-score[f][pool], kind="stable")] for f in range(K)}
    ...
    B = _bm25(desc, fam); rec("BM25", rank({f: B[f] for f in range(K)}))
    ...
    for tag, nm in [("", "stock"), ("_i0258be9d", "I-competent"), ("_i06c7d226", "I-demonstrated")]:
        ...
        rec(f"hybrid ({nm})", {f: pool[np.argsort(-_rrf(B[f][pool], (Eq @ Fq[f])[pool]), kind="stable")] for f in range(K)})
        T = np.load(f"{p}/{base}{tag}.npy"); rec(f"SOTA+rerank ({nm})", {f: T[f][T[f] >= 0] for f in range(K)})

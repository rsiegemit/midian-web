...
def data():
    ...
    keep = d.eval_name.map(d.eval_name.value_counts()) >= MIN_FAMILY
    d = d[keep].reset_index(drop=True)
    M = [c for c in d.columns if c + "|total_cost" in d.columns]
    return (
        d.prompt.astype(str).to_numpy(),
        d.eval_name.to_numpy(),
        d[M].to_numpy(float),
        d[[m + "|total_cost" for m in M]].to_numpy(float),
        M,
    )
...
def embed(prompts):
    ...
    e = SentenceTransformer("all-MiniLM-L6-v2", device="cpu").encode(
        list(prompts), batch_size=64, show_progress_bar=False, normalize_embeddings=True
    )
    np.save(EMB, e.astype(np.float32))
    return e


def midian_tree_pick(score, r=R_MIDIAN):
    """The pick of MIDIAN w/o defenses over n agents with scores (n,): cohorts of r by index, best per cohort promoted,
    until one."""
...
def aiq(pts, c_lo, c_hi, hull=False):
    ...
    if hull:  # zero router: linear interpolation between single models
        o = np.argsort(pts[:, 0])
        c, q = pts[o, 0], np.maximum.accumulate(pts[o, 1])
...
def main():
    ...
    fams = np.unique(fam)
    F = {f: i for i, f in enumerate(fams)}
    fi = np.array([F[f] for f in fam])
    rows, curves = [], []
    for seed, (tr, te) in enumerate(
            ...
            rows.append(
                {
                    "seed": seed,
                    "router": name,
                    "aiq": aiq(pts, c_lo, c_hi, hull),
                    "quality_at_lambda0": pts[0, 1] if not hull else pts[:, 1].max(),
                    "labelled_outcomes": extra if extra is not None else len(tr) * len(M),
                }
            )
            for lam, (c, q) in zip(LAMBDAS if not hull else [np.nan] * len(pts), pts):
                curves.append({"seed": seed, "router": name, "lam": lam, "cost": c, "perf": q})

        # oracle: cheapest correct model per prompt (their definition); one point
        best = P.max(1, keepdims=True)
        ocost = np.where(P >= best, C, np.inf)
        opick = ocost.argmin(1)
        rows.append(
            {
                "seed": seed,
                "router": "oracle",
                "aiq": np.nan,
                "quality_at_lambda0": P[np.arange(len(te)), opick].mean(),
                "labelled_outcomes": np.nan,
            }
        )
        curves.append(
            {
                ...
                "router": "oracle",
                "lam": np.nan,
                "cost": C[np.arange(len(te)), opick].mean(),
                "perf": P[np.arange(len(te)), opick].mean(),
            }
        )
        ...
        add(
            "mlp",
            MLPRegressor(hidden_layer_sizes=(128,), max_iter=200, random_state=seed)
            .fit(e[tr], perf[tr])
            .predict(e[te]),
        )
        ...
        for b in B_PROBES:
            probe = np.concatenate(
                [rng.choice(tr[fi[tr] == k], size=min(b, (fi[tr] == k).sum()), replace=False) for k in range(len(fams))]
            )
            est = np.stack(
                [perf[probe][fi[probe] == k].mean(0) for k in range(len(fams))]
            )  # (F, M) per-family accuracy
            ...
            add(
                f"knn_b{b}",
                KNeighborsRegressor(n_neighbors=min(KNN_K, len(probe))).fit(e[probe], perf[probe]).predict(e[te]),
                extra=len(probe) * len(M),
            )  # same labels, their router
    ...
    df.to_csv(f"{OUT}/aiq_rows.csv", index=False)
    cv.to_csv(f"{OUT}/curves.csv", index=False)
    agg = (
        df.groupby("router")
        .agg(
            aiq=("aiq", "mean"),
            aiq_sd=("aiq", "std"),
            q0=("quality_at_lambda0", "mean"),
            labelled=("labelled_outcomes", "mean"),
        )
        .sort_values("aiq", ascending=False)
    )
    md = [
        "# RouterBench on its own terms (5 stratified 70/30 splits; local MiniLM embeddings for every router)",
        "",
        f"families {len(fams)}, prompts {len(prompts)}, models {len(M)}; AIQ = normalised area under the "
        f"non-decreasing quality-vs-cost envelope over the single-model cost range",
        "",
...
def fig(cv, agg):
    ...
    show = [
        ...
        "probe_family_b20",
        "probe_family_b50",
        "probe_family_b20_oraclefamily",
        "knn_b20",
    ]
    f, ax = plt.subplots(figsize=(7.5, 5))
    for r in show:
        sel = cv.router == r
        key = (cv.lam if r != "zero_router" else cv.cost.round(9))[sel]
        ...
        if r == "zero_router":  # single models as points, their non-decreasing hull as the line
            ...
            ax.plot(
                g.cost,
                np.maximum.accumulate(g.perf),
                color="#7f8c8d",
                ls="--",
                lw=1.0,
                label=f"{r} = hull (AIQ {agg.loc[r, 'aiq']:.3f})",
            )
            continue
        ax.plot(
            g.cost,
            ...
            marker="o",
            ms=3,
            lw=1.4 if "probe" in r else 1.0,
            label=f"{r} (AIQ {agg.loc[r, 'aiq']:.3f})",
        )
    o = cv[cv.router == "oracle"][["cost", "perf"]].mean()
    ax.plot(o.cost, o.perf, "k*", ms=12, label="oracle (cheapest correct)")
    ax.set_xscale("log")
    ax.set_xlabel("mean $ per prompt (test)")
    ax.set_ylabel("mean performance (test)")
    ax.grid(alpha=0.3)

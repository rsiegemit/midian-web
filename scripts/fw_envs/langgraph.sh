...
source "$CONDA_SH"
PREFIX="$RTE_DATA/env/fw_langgraph"
...
PTH
"$PREFIX/bin/pip" install --no-input -q -r "$REPO/requirements-frameworks/langgraph.txt"
"$PREFIX/bin/python" -c "import requests; print('fw_langgraph OK')"

...
Reads figures/shortlist/live.csv (per framework x shortlist x cell: seed mean and 95% CI; written by shortlist_figs.py).
  E  live specialist, n = 10^2 .. 10^5: per n, one bar per shortlist = the MEAN over frameworks (solid honest, hatched
     beta = 0.5 low-skill cartel), a black dot = the BEST single framework on that shortlist; oracle dotted and MIDIAN-VA
     (whole population) solid, as lines over each group.
  F  the same at n = 10^5 only (every shortlist ran there), shortlists sorted by the honest mean.
...
def pair(ax, x, w, q, src, label):
    ...
    for h, r in enumerate(REG):
        ...
        if h == 0: drawn = True
        ax.plot(xx, v["best"], "o", ms=2.2, color="black", zorder=3)
        if v["star"]: ax.text(xx, max(v["mean"], v["best"]) + 0.008, "*", ha="center", va="bottom", fontsize=6, zorder=5)
    return drawn
...
def fig_E(s, ref):
    ...
    for i, n in enumerate(ns):
        ...
        for j, src in enumerate(srcs):
            ...
            if pair(ax, i + (2 * j + 1 - len(srcs)) * w, w, q, src, None if src in seen else SHORT[src]): seen.add(src)
        lines(ax, ref, n, i - 0.46, i + 0.46, i == 0)
    ax.plot([], [], "o", ms=2.5, color="black", label="best framework")
    ax.set_xticks(range(len(ns))); ax.set_xticklabels([f"n = {n:,}" for n in ns]); finish(ax, fig, s,
        "E  frameworks by shortlist (live, specialist): bar = mean of frameworks, dot = best; solid honest, hatched cartel; * = not in yet", "E_shortlists_by_n", 6)
...
def fig_F(s, ref, n=100000):
    ...
    finish(ax, fig, q, f"F  every shortlist at n = {n:,}, best to worst: bar = mean of frameworks, dot = best; solid honest, hatched cartel", "F_shortlists_1e5", 3)


def finish(ax, fig, s, title, name, ncol):
    ax.set_ylim(0.2, 0.95); ax.set_ylabel("success"); ax.grid(axis="y", lw=0.3, alpha=0.35); ax.set_axisbelow(True)
    ax.set_title(title)
    ax.legend(ncol=ncol, frameon=False, loc="lower center", bbox_to_anchor=(0.5, 1.07))
...
if __name__ == "__main__":
    s, ref = summarise(load())
    fig_E(s, ref); fig_F(s, ref)

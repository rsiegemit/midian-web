...
NICE = {"midian": "midian", "midian{V}": "midian_v", "midian{r:5,V}": "midian_v_r5", "midian{r:5}": "midian[r=5]",
        "flat_probe_argmax": "flat_probe_argmax_frozen", "flat_probe_argmax{online:true}": "flat_probe_argmax_online", "declared_argmax": "declared_argmax",
        "llm_supervisor": "llm_supervisor", "oracle": "oracle", "random": "random", "sequential_halving": "sequential_halving", "sequential_halving{peer}": "sequential_halving_peer"}
...
for off, m, lab, c in ((-.3, "oracle", "oracle", GRY), (-.1, "midian{V}", "midian_v", ORG), (.1, "midian", "midian", RED), (.3, "flat_probe_argmax", "flat_probe_argmax_frozen", "#7f8c8d")):
...
for ax, n in zip(axes, ["n100", "n1000"]):
    ...
    for off, suf, lab, col in ((-.27, "{}", "own selection (TF-IDF top-10)", "#2980b9"), (0, '{"r":10,"retrieval":"midian"}', "+ midian_v cohort r=10", ORG), (.27, '{"r":5,"retrieval":"midian"}', "+ midian_v_r5 cohort", YEL)):
        ax.bar(x + off, [c[f + suf].mean() for f in fws], .27, label=lab, color=col)
    mv = c['midian{"cached":true,"verify":true}'].mean(); ax.axhline(mv, color=RED, ls="--", label=f"midian_v alone ({mv:.2f})"); ax.axhline(c["oracle{}"].mean(), color=GRY, ls=":", label="oracle")
...
for ax, ls in zip(axes, ["random", "low_skill_first"]):
    ...
    for m, c, lab in [("oracle", GRY, "oracle"), ("sequential_halving", "#2c3e50", "sequential_halving (trusted observer)"), ("sequential_halving{peer}", "#8e44ad", "sequential_halving_peer (fair control)"),
                      ("midian{V}", ORG, "midian_v"), ("midian{r:5,V}", YEL, "midian_v_r5"), ("midian", RED, "midian"), ("flat_probe_argmax", BLU, "flat_probe_argmax_frozen")]:
...
LABEL = {"midian{r:5}": "midian[r=5]", "midian{V}": "midian_v", "midian{r:5,V}": "midian_v_r5", "midian_llm_descent": "midian_llm_descent", "midian{online:false}": "midian[online=False]",
         "sequential_halving": "sequential_halving (trusted observer)", "sequential_halving{peer}": "sequential_halving_peer", "flat_probe_argmax{online:true}": "flat_probe_argmax_online",
         "flat_probe_argmax": "flat_probe_argmax_frozen"}
...
if runs:
    ...
    for m, lab, c in [('midian{"cached":true,"verify":true}', "midian_v", ORG), ("midian{}", "midian", RED), ('midian{"online":false}', "midian[online=False]", "#e74c3c"),
                      ('flat_probe_argmax{"online":true}', "flat_probe_argmax_online", BLU), ("warm_start_bandit{}", "warm_start_bandit", "#27ae60"), ("llm_supervisor{}", "llm_supervisor", "#8e44ad")]:
        arr = np.array([r[m] for r in runs], float).reshape(len(runs), -1, B).mean(axis=(0, 2)) - orc
        ax.plot(np.arange(len(arr)) * B + B / 2, arr, marker="o", label=lab, color=c, ls="--" if "online=False" in lab else "-", lw=2.5 if lab == "midian" else 1.2)
...
KEEP = {"midian": ("midian", RED, "-"), "midian{V}": ("midian_v", ORG, "-"), "flat_probe_argmax": ("flat_probe_argmax_frozen", BLU, "-"), "declared_argmax": ("declared_argmax", "#16a085", "--"),
        "cnp_self_bid": ("cnp_self_bid", "#8e44ad", ":"), "sequential_halving": ("sequential_halving", "#2c3e50", "-"), "gossip_reputation_greedy": ("gossip_reputation_greedy", "#27ae60", "-")}
...
plt.suptitle("F3: cost vs n on the calibrated bernoulli world, 10² to 10⁷: midian per task = r·⌈log_r n⌉ comparisons (∝ n^0.11), midian_v 1, flat/declared/CNP = n")
...
for m, lab, c in [("oracle", "oracle", GRY), ("sequential_halving", "sequential_halving (trusted)", "#2c3e50"), ("midian{V}", "midian_v", ORG), ("midian", "midian", RED), ("flat_probe_argmax", "flat_probe_argmax_frozen", BLU),
                  ("warm_start_bandit", "warm_start_bandit", "#27ae60"), ("declared_argmax", "declared_argmax", "#16a085"), ("ucb_per_family", "ucb_per_family", "#8e44ad")]:
...
for m, lab, c in [("midian{delta:0.3333333333333333,r:5}", "midian[r=5,δ=1/3]", "#e74c3c"), ("midian{delta:0.3333333333333333,r:10}", "midian[r=10,δ=1/3]", RED), ("midian{delta:0.3333333333333333,r:20}", "midian[r=20,δ=1/3]", "#7b241c"),
                  ("midian{delta:0.0,r:10}", "midian[r=10,δ=0] (no trim)", ORG), ("midian{r:5,V}", "midian_v_r5", YEL), ("midian{r:10,V}", "midian_v", "#f39c12"), ("midian{r:20,V}", "midian_v[r=20]", "#b9770e"),
                  ("flat_probe_argmax", "flat_probe_argmax_frozen", BLU), ("oracle", "oracle", GRY)]:

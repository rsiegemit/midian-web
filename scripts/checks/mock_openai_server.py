...
{"next_speaker": ..., "assignee_id": ..., "choice": 1 (1-based, LlamaIndex), "index": 0}). Usage: python
...
def _fill(schema, first, root=None, agentish=False):
    ...
    for name, spec in (schema.get("properties") or {}).items():
        ...
        if t == "boolean":
            ...
            out[name] = []
        elif t == "object" or spec.get("properties"):
            out[name] = _fill(spec, first, root, mine)
        elif mine:
            out[name] = first
        else:
            out[name] = "mock"
...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if tools:
            ...
            pick = next(
                (t for t in tools if first in json.dumps(t)),
            ...
            sch = _schema(req)
            msg["content"] = json.dumps(_fill(sch, first, sch))
            finish = "stop"
        else:
            body = {
                "next_speaker": first,

...
def main() -> int:
    ...
    for p in b.profiles:
        print(f"  agent {p['id']}: {p['model']} tool={p['tool']} " f"specialty={[FAMILIES[f] for f in p['specialty']]}")
    ...
    t0 = time.time()
    out = b.execute_many(np.repeat(np.arange(N), K), np.tile(np.arange(K), N), 1, np.random.default_rng(0))
    dt = time.time() - t0

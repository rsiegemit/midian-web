...
def vprobes(n, r=10):
    """MIDIAN w/o audits build probes: n*K*(B-1) at level 0 + e per forwarded candidate, e = floor(n / C), C = child
    slots above level 0."""
    C, m = 0, math.ceil(n / r)
    while m > 1:
        C += m
...
EXPECT = {
    ...
    "midian_wo_defenses": (
        lambda n: dict(probes=n * K * B, reports=n * K * B * 9),
        lambda n: dict(
            hops=math.ceil(math.log10(n)),
            comparisons=10 * math.ceil(math.log10(n)),
            messages=2 * math.ceil(math.log10(n)),
        ),
    ),
    "midian_wo_audit": (
        lambda n: dict(probes=vprobes(n), reports=vprobes(n) * 9),
        lambda n: dict(hops=0, comparisons=1, messages=2),
    ),
    ...
    "referral_network": (
        lambda n: dict(probes=n * K * B, reports=n * K * B, messages=n * 10),
        lambda n: dict(hops=4, comparisons=40, messages=80),
    ),
    "gossip_reputation_greedy": (
        lambda n: dict(probes=n * K * B, reports=n * K * B),
        lambda n: dict(hops=6, comparisons=60, messages=120),
    ),
    "flat_nsw_router": (
        lambda n: dict(probes=n * K * B, messages=0),
        lambda n: dict(hops=math.ceil(math.log2(n)), comparisons=50, messages=0),
    ),
}
ARMS = {
    "midian_wo_defenses": ("midian", {"audit": False, "verify": False}),
    "midian_wo_audit": ("midian", {"audit": False}),
}  # label -> (method, params)
...
def run(name, n, **kw):
    ...
    M.build(v, Budget(B))
    build = w.ledger.diff(s0)
    ts = w.tasks(Q)
    s1 = w.ledger.snapshot()
    ok = []
    for t in ts:
        a = M.fetch(t)
        a = a if isinstance(a, (int, np.integer)) else a[0]
        assert 0 <= a < n, (name, a)
        o = w.execute(a, t)
        M.observe(t, a, o)
        ok.append(o)
...
def exact_argmax(name, n):
    ...
    M = make(name)
    v = w.view(M.needs)
    v.probe_many = lambda agents, fams, reps: np.broadcast_to(
        w.S[np.asarray(agents), np.asarray(fams)][..., None],
        np.broadcast_arrays(np.asarray(agents), np.asarray(fams))[0].shape + (reps,),
    ).astype(np.float64)
...
if __name__ == "__main__":
    ...
    for name in ap.parse_args().methods:
        for n in (100, 1000):
            ...
            print(
                f"{name:22s} n={n:5d} succ={succ:.3f} rnd={rnd:.3f} build={ {k:v for k,v in build.items() if v} } "
                f"fetch={ {k:round(v,3) for k,v in fetch.items() if v} }  "

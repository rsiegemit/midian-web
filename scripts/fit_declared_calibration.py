...
IN-CODE table (rte.backends.calibrated_declared) against live: mean, spread, corr(S, D), mass at 0 and 1, and the one number
...
def load(shape):
    out = [(np.load(f"{d}/S.npy"), np.load(f"{d}/D_self_described.npy")) for n in (100, 1000)
           for d in sorted(glob.glob(f"{POP}/{shape}_n{n}_K16_seed*")) if os.path.exists(f"{d}/D_self_described.npy")]
...
print(f"live specialist n = 100 + 1,000: {npop} populations, {S.size:,} (agent, family) pairs; rating values {np.unique(D).tolist()}")
...
for k in range(10): print("    [" + ", ".join(f"{p:.4f}" for p in P[k]) + "],")
print("])")
if not np.allclose(P, CAL_P, atol=5e-5): print("!! the table in rte/backends/__init__.py differs from this fit: paste the one above")

# ---- the rejected alternative: censored-normal linear model, fitted by maximum likelihood
def nll(th):
    a, c, ls = th; s = np.exp(ls); mu = a + c * S64; lo, hi = D64 <= 0, D64 >= 1; mid = ~(lo | hi)
    return -(stats.norm.logcdf(-mu[lo] / s).sum() + stats.norm.logsf((1 - mu[hi]) / s).sum()
             + (stats.norm.logpdf((D64[mid] - mu[mid]) / s) - ls).sum())
a, c, ls = optimize.minimize(nll, [0.5, 0.3, np.log(0.3)], method="Nelder-Mead", options={"xatol": 1e-5, "fatol": 1e-3}).x
...
r = D64 - np.polyval(ols, S64); agent = np.repeat(np.arange(S.shape[0]), S.shape[1])
print(f"between-agent share of the OLS residual variance (NOT modelled; draws are i.i.d.): "
...
for shape in ("specialist", "heavy_tail", "bimodal"):
    ...
    print(f"{shape} ({k} populations){'' if shape == 'specialist' else ' -- out of sample: table fitted on specialist'}")
    show("live self-described", summary(S_, D_, np.random.default_rng(0)))
    for seed in (1, 2):
        show(f"calibrated (in-code table), seed {seed}", summary(S_, calibrated_declared(S_, seed), np.random.default_rng(0)))
    show("Tobit linear (rejected)", summary(S_, tobit(S_, np.random.default_rng(1)), np.random.default_rng(0)))
    show("programmatic clip(S + N(0, .05))", summary(S_, np.clip(S_ + np.random.default_rng(1).normal(0, .05, S_.shape), 0, 1), np.random.default_rng(0)))

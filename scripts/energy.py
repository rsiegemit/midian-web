...
ABL = ("midian_wo_defenses", "midian_wo_verify", "midian_wo_audit")          # MIDIAN's ablations, kept apart from its other variants
...
def rows(*grids, n=1000):
    ...
    df = load(list(grids)); df = df[(df.declared_source == "self_described") & (df.n == n)]; df["m"] = np.where(df.label.isin(ABL), df.label, df.method + df.params.str.replace("{}", ""))   # MIDIAN ablations by label
...
if __name__ == "__main__":
    ...
    fws = [m for m in t.index if m.startswith("fw_")]; mids = ["midian_wo_defenses", "midian_wo_verify", "midian_wo_audit"]
    ...
    md += ["", "In MESSAGES (fetch 2 per level + observe-time update 1 per level, commit 3415f03) the crossing is immediate: MIDIAN w/o defenses 1,010 + 9t vs a framework 1,000 + 12t crosses at t = " + f"{crossing(t, 'midian_wo_defenses', 'fw_autogen', 'build_msgs', 'msgs_per_task'):.0f}; MIDIAN w/o audits (1,010 + 5t) at t = {crossing(t, 'midian_wo_audit', 'fw_autogen', 'build_msgs', 'msgs_per_task'):.0f}. "
           "In COMPARISONS MIDIAN w/o defenses pays 60 per task (30 descent + 30 observe-time update) vs a framework's 10 (MIDIAN w/o audits 31 = 1 cached pick + 30 update; halving 1; flat 1,000), so no MIDIAN variant undercuts a framework on comparisons; the saving of MIDIAN w/o audits over MIDIAN w/o defenses is the descent, not the update. "
           "Per task, MIDIAN's cost is communication (messages and comparisons), not LLM compute: it makes no LLM call at route time."]
    md += ["", "The 48,000-probe build of MIDIAN w/o defenses by population shape (GPU-s): " + ", ".join(f"{d} {48000 * v:,.0f}" for d, v in probe_cost.items())
           + "; the crossings scale with it (heavy_tail and bimodal cross ~3x sooner).",
           "Reading: against a one-call framework (AutoGen) MIDIAN w/o defenses breaks even after ~9,400 tasks on specialist (~2,900 on heavy_tail), MIDIAN w/o verification after ~9,900; against the multi-call frameworks (CrewAI, LlamaIndex, CAMEL) after 1,900-2,300 tasks; against Magentic-One after ~990 (7B) / ~590 (14B arm). Before the crossing the framework is cheaper; after it, the probe-based methods' cost is flat while every framework's keeps growing linearly."]
    md += ["", "## Combined currencies (*)", "",
           ...
           f"(b) LATENCY on the critical path per task: each sequential message hop = {RTT*1e3:.0f} ms RTT (MIDIAN w/o defenses: the 2·depth = 6 fetch hops; the observe-time update propagation (1 message per level) is off the critical path and excluded; MIDIAN w/o audits 2 ms; frameworks and llm_supervisor: 2 hops + the supervisor call at its measured median latency under shared-fleet load); comparisons {T_CMP*1e9:.0f} ns each (flat's 1,000 = 10 us); a route-time probe call (verify_on_claim) {PROBE_LAT:.1f} s.", "",

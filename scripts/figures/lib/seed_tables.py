...
def _table(df, pooled_shapes=False):
    ...
    if df.empty:
        return {}
    df = df.assign(label=[label(m, p) for m, p in zip(df.method, df.params)], regime=[ab(b, l) for b, l in zip(df.beta, df.liar_select)])
...
def tables():
    sw = switched()
    E30 = grids.ERRATUM30
    spec = [(("live", "specialist", n), grids.LIVE_GRIDS[n] + grids.b_grids(t) + [f"tuned_wsb_{t}", f"pool_seeds_{t}"], "live")
            for n, t in grids.SIZE.items()]
    spec += [(("routereval", "strong_to_weak", 5000), [E30["routereval"]] if "routereval" in sw
              else ["routereval_mmlu5k"] + grids.b_grids("routereval5k"), None),
             (("llmrouterbench", "20 models", 20), [E30["llmrouterbench"]] if "llmrouterbench" in sw
              else ["llmrouterbench_pool"] + grids.b_grids("llmrouterbench"), None),
             (("bernoulli", "specialist", 10 ** 7), [grids.MATRICES["bernoulli"]] + grids.b_grids("bernoulli_1e7")
              + ([E30["bernoulli"]] if "bernoulli" in sw else []), "specialist"),
             (("replay", "all shapes pooled", 10 ** 6), [E30["replay"]] if "replay" in sw
              else [grids.MATRICES["replay"]] + grids.b_grids("replay_1e6"), "pooled")]
    out = {}
    for (fam, grp, n), gs, kind in spec:
        ...
        if not df.empty:                                     # TrueSkill: post probe-duplicate-fix rows only (erratum 30)
            df = df[(df.method != "trueskill_per_family") | df.grid.str.startswith("trueskill_fix_") | df.grid.isin(list(E30.values()))]

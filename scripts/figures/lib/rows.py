"""Reading stored result rows, and the per-row helpers every figure / number script shares.
    ...
    load_fw(grid)             read_rows de-duplicated the way the framework-shortlist scripts count (every row, b included)
...
def read_rows(grid, usecols=None, where=None, rowsd=True, csv_first=False):
    ...
    d = f"{RESULTS}/{grid}"
    fr = [keys.normalize(pd.DataFrame([{**json.load(open(f)), "rid": os.path.basename(f)[:-5]}   # the file name IS the rid
                                       for f in glob.glob(f"{d}/rows.d/*.json")]), d)] if rowsd else []
    if os.path.exists(f"{d}/rows.csv"):
        ...
        if where is None:
            ...
            fr.append(pd.concat(where(keys.normalize(c, d)) for c in
                                pd.read_csv(f"{d}/rows.csv", usecols=cols, chunksize=500000, low_memory=False)))
...
def load_fw(grid):
    ...
    return df.drop_duplicates([c for c in ("n", "b", "dist", "beta", "liar_select", "seed", "method", "params") if c in df])

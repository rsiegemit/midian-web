...
FIXED = [
    ...
    "declared_argmax",
]  # C: the lines; the pools as FLAT_SCAN / the band
SHOW = set(FIXED) | set(LEARNED) | set(BANDIT)  # + every member of the two pools


def costs(refresh=False):
    if os.path.exists(COST) and not refresh:
        return pd.read_csv(COST)
    keep = lambda c: c[
        ((c.b == 3) | ((c.method == "midian") & (c.params == "{}"))) & (c.beta == 0)
    ]  # MIDIAN at every b: D's build ledger
...
def va_build(d):
    ...
    L = pd.concat(
        [
            f
            for f in (read_rows(g, ("rid", "n", "b", "method", "params", "build_probes")) for g in VA_GRIDS)
            if not f.empty
        ]
    )
    L = L.drop_duplicates("rid")  # a row can sit in both rows.d and rows.csv
    L = (
        L[(L.method == "midian") & (L.params.fillna("{}").astype(str) == "{}")]
        .groupby(["n", "b"])
        .build_probes.median()
    )  # the full method
    for (n, b), v in d[d.label == "midian"].set_index(["n", "b"]).build_probes.items():
        L.loc[(n, b)] = L.get((n, b), v)
    ratio = {b: float((L.xs(b, level="b") / (L.xs(b, level="b").index * 16 * b)).median()) for b in (1, 3, 5)}
    return {
        (n, b): (
            (float(L[(n, b)]), "ledger")
            if (n, b) in L.index
            else (ratio[b] * n * 16 * b, f"ratio {ratio[b]:.3f} x nKb")
        )
        for n in (1000, 100000, 10**7)
        for b in (1, 3, 5)
    }
...
FLAT_SCAN = ["flat_probe_argmax_online", "declared_argmax"] + POOLS[
    "best_bandit"
]  # one line: every one scans all n agents
...
def draw_C(ax, d):
    ...
    d = d[d.b == 3].assign(work=lambda x: x.messages_per_task + x.comparisons_per_task)
    rec = []
    hs, ls = [], []
    ...
    print(
        f"[C] any flat scan: members {sorted(set(d[d.label.isin(FLAT_SCAN)].label))} differ by at most {dev:.1%} at "
        f"any n"
    )
    for k in ("midian", "midian_wo_defenses", "flat_scan"):
        ...
        s = slope(q)
        rec += [dict(arm=S.NAME[k], key=k, n=int(n), work=w, slope=s) for n, w in zip(q.n, q.work)]
        ax.plot(q.n, q.work, "o-", ms=3, lw=1.4, color=S.COLOR[k])
        hs.append(Line2D([], [], color=S.COLOR[k], marker="o", ms=3, lw=1.4))
        ls.append(f"{S.NAME[k]} ({growth(s)})")
    ...
    for e in ("min", "max"):
        ax.plot(q.n, q[e], "-", lw=1.0, color=c)
    a, z = sorted((lo, hi))
    span = (
        growth(lo)
        if growth(lo) == growth(hi)
        else rf"$\propto n^{{{a:.2f}}}$ to $n^{{{z:.2f}}}$"
        if a >= 0.3
        else f"{growth(lo)} to {growth(hi)}"
    )
    hs.append(Patch(facecolor=c, alpha=0.3, edgecolor=c, lw=1.0))
    ls.append(f"{S.NAME[k]} ({span})")
    rec += [
        dict(arm=S.NAME[k], key=k, n=int(n), work=w, work_max=m, slope=lo, slope_max=hi)
        for n, w, m in zip(q.n, q["min"], q["max"])
    ]
    ax.set_xscale("log")
    ax.set_yscale("log")
...
def compute_D(d):
    ...
    fw = energy.table()
    fw = fw[fw.index.str.startswith("fw_") & ~fw.index.str.contains("supervisor")]
    fwJ = fw.per_task_J  # supervisor call energy + the framework's messages / comparisons
    PJ = energy.probe_cost["specialist"] * 700
    rec = []
    va = d[(d.label == "midian") & (d.b == 3)].set_index("n")
    VB = va_build(d)
    for n in N_SHADE:
        r = va.loc[n]
        marg = r.messages_per_task * energy.J_MSG + r.comparisons_per_task * energy.J_CMP
    ...
    refs = pd.DataFrame(
        [
            dict(figure="D_energy_per_query", key=k, value=v, text=S.FW_NAME.get(m, m))
...
def render_D(ax, rec, refs):
    ...
    for i in range(0, len(rec), 2):  # one curve per (n, b), then its two break-evens
        ...
        ax.plot(
            T,
            r.build_J / T + r.marginal_J,
            S.B_LINESTYLE[int(r.b)],
            lw=1.2,
            color=S.shade(S.COLOR["midian"], N_SHADE[int(r.n)]),
            zorder=3,
        )
...
def main(argv=None):
    ...
    p.add_argument(
        "--refresh", action="store_true", help="re-read the bernoulli rows into results/aggregates/cost_by_n.csv"
    )
    ...
    draw_C(ax, d).to_csv(f"{out}/C_routing_work_vs_n.csv", index=False)
    S.save(f, "C_routing_work_vs_n", out)

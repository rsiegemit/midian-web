...
def body(s):
    ...
    for n, q in s.groupby("n"):
        ...
        pick = {k: max((v for v in vs if v in h.index), key=h.get) for k, vs in VARIANTS.items() if any(v in h.index for v in vs)}
        out += [q[q.shortlist.isin([k for k in S.SHORTLIST_BODY if k not in VARIANTS])].assign(variant=lambda x: x.shortlist)]
...
def lines(ax, ref, n, x0, x1):
    ...
    if pd.notna(o.get("random")): S.ref(ax, x0, x1, o["random"], "random")
...
def pair(ax, x, w, q, key, dots=False):
    ...
    for h, r in enumerate(REG):
        ...
        v = q.loc[r]; INCOMPLETE[0] |= bool(v["star"]); xx = x + (h - 0.5) * w
...
def fig_G(d, out, n=100000):
    ...
    for (r, src), q in fw[fw.shortlist != "tfidf"].groupby(["regime", "shortlist"]):
        ...
        k = len(diff)
        if k < MIN_FW: INCOMPLETE[0] = True; continue            # too few paired frameworks: left out
        half = diff.std(ddof=1) / np.sqrt(k) if k > 1 else np.nan   # +/- 1 s.e. across frameworks
        rows.append(dict(regime=r, shortlist=src, name=S.SHORTLIST_APPENDIX[src], lift=diff.mean(), half=half, frameworks=k,
    ...
    fig, ax = S.figure("appendix"); w = 0.38
    for i, src in enumerate(order):
        for h, r in enumerate(REG):
            q = t[(t.shortlist == src) & (t.regime == r)]
            if q.empty: INCOMPLETE[0] = True; continue
            v = q.iloc[0]; xx = i + (h - 0.5) * w; INCOMPLETE[0] |= bool(v.star)
            S.bar(ax, xx, v.lift, w * 0.95, src, cartel=bool(h)); S.whisker(ax, xx, v.lift, v.lift - v.half, v.lift + v.half)

...
Do-not-add list (lib/exclusions.py) applies; MIDIAN cohorts other than r = 10, the shuffled MIDIAN cohort (a position control), the lying-text condition and the 14B
...
# where the oracle and MIDIAN for a condition come from: grids.REF_GRIDS (every grid at that n pooled, matched on (dist, regime, seed))
_mem = {}
...
def collect(family):
    ...
    for grid in sorted(os.path.basename(g) for g in glob.glob(f"{R}/*") if shortlist_grid(family, os.path.basename(g), family == "routereval" and h30())):
        df = rows(grid)
...
def draw(family, key, cell, csv_rows):
    ...
    for m, lbl, ls in (("oracle", "oracle", ":"), ("midian", S.NAME["midian_ref"], "-"), ("random", "random", None)):   # reference lines
        if m not in cell["ref"]: continue
        s = cell["ref"][m]; lo, hi = _ci(s); v = float(s.mean()); c = S.EXPLORE_COLOR.get(m, "#555")
    ...
    ax.set_xticks(xfw); ax.set_xticklabels([S.ABBR.get(m, m) for m in fws], rotation=30, ha="right", rotation_mode="anchor")
...
def main(argv=None):
    ...
    for family in families:
        ...
        pd.DataFrame(csv_rows, columns=["family", "regime", "dist", "n", "arm", "shortlist", "mean", "ci_lo", "ci_hi", "seeds",
                                        "rerun_outstanding"]).to_csv(f"{CSV_OUT}/{family}.csv", index=False)

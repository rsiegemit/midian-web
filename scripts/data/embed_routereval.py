...
setting they use, it calls FrameworkMethod._index -- the exact code build() runs -- with RTE_EMBED_CACHE_DIR set.
    python scripts/embed_routereval.py re_sl_embed_small re_sl_embed_1k re_sl_embed_5k            # GPU job: fill
    CUDA_VISIBLE_DEVICES= python scripts/embed_routereval.py --check re_sl_embed_small ...          # CPU: every file hit
...
ap = argparse.ArgumentParser(); ap.add_argument("grids", nargs="+"); ap.add_argument("--check", action="store_true")
a = ap.parse_args()
...
for g in a.grids:
    for blk in blocks(cfg, g):
        ...
        for s in method_specs(blk):                     # the nine frameworks share texts and retrieval: one per setting
            if s["params"].get("retrieval") in C.DENSE: specs.setdefault(jkey(s["params"]), s)
        units += [(g, cell, seed, list(specs.values())) for cell in cells(blk) for seed in seeds(blk["seeds"])]
sota = C.FrameworkMethod._sota_table
# two passes so a 20 GB GPU slice suffices: the embedder alone (shortlist tables skipped), freed, then the reranker alone
for stage in (("embed", "rerank") if not a.check else ("check",)):
    ...
    for g, cell, seed, specs in units:
        ...
        print(f"[{stage} {time.time() - t0:.0f}s] {g} n={cell['n']} beta={cell['beta']} seed={seed}: {len(specs)} settings "
              f"-> {m._popdir(world.view(m.needs))}", flush=True)
    if stage == "embed":
        import torch
        from rte.methods._learned import _models
        _models.clear(); torch.cuda.empty_cache()

...
FrameworkMethod._embeddings looks for, so the routing jobs never load the embedder. --sota also pre-warms the reranked
(K, k) shortlist, which is what keeps the sota routing jobs off the GPU entirely. Needs a GPU for anything larger than
MiniLM; run it as a SLURM job, never on the login node. Re-running is free: finished populations are skipped, and each
file is written atomically so concurrent array tasks cannot corrupt one another."""
...
def population_dirs(dists, ns, seeds):
    ...
    for d in sorted(os.listdir(POP)):
        p = f"{POP}/{d}"
        if not os.path.isdir(p) or not os.path.exists(f"{p}/descriptions.json"):
            continue
        try:
...
def main():
    ...
    ap.add_argument("--ns", nargs="*", type=int, default=[])
    ...
    ap.add_argument(
        "--instruct",
        default="",
        help="query-side task instruction for an asymmetric embedder (only the K family texts depend on it)",
    )
    a = ap.parse_args()
    sl = slug(a.model)
    ...
    def pending(p):  # --sota must not be skipped just because embeddings exist
        return (
            not os.path.exists(f"{p}/descriptions_{sl}.npy")
            or not os.path.exists(f"{p}/{fam_name}")
            or (a.sota and not os.path.exists(f"{p}/{sota_name}"))
        )

    todo = [p for p in dirs if pending(p)]
    print(
        f"model {a.model} (local: {resolve(a.model)})\n{len(dirs)} populations in shard, {len(todo)} to do", flush=True
    )
    if a.list:
        for p in todo:
            print("  ", os.path.basename(p), len(json.load(open(f"{p}/descriptions.json"))))
        return
    ...
    for i, p in enumerate(todo, 1):
        ...
        t0 = time.time()
        E = (
            np.load(f"{p}/descriptions_{sl}.npy")
            if os.path.exists(f"{p}/descriptions_{sl}.npy")
            else embed(desc, a.model)
        )
        save(f"{p}/descriptions_{sl}.npy", E)
        dt = time.time() - t0
        print(
            f"[{i}/{len(todo)}] {os.path.basename(p):38s} {len(desc):7,d} texts  {dt:7.1f}s  "
            f"{len(desc)/max(dt,1e-9):7.1f}/s",
            flush=True,
        )
        ...
        T = sota_shortlist(
            _bm25(desc, fam),
            E,
            np.load(f"{p}/{fam_name}"),
            pool,
            desc,
            fam,
            lambda q, d: np.asarray(ce.predict([(q, x) for x in d]), dtype=np.float32),
            a.k,
            a.rerank_pool,
        )
        save(out, T)

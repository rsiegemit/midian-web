...
def test_churn_replaces_in_place_and_reset_restores():
    w = World(**W)
    S0, D0, L0 = w.S.copy(), w.D.copy(), w.liars.copy()
    ...
    ids2 = World(**W).churn(0.1)
    assert (ids2 == ids).all()                 # same draw for every method
    w.reset()
...
def test_epoch_rule_stale_route_scores_zero_once():
    ...
    a = int(ids[0])
    w.S[a] = 1.0
    w.backend._S[a] = 1.0                                          # would succeed if it were known
    assert w.execute(a, t) == 0 and w.execute(a, t) == 1                         # stale once, then real
    b = int(ids[1])
    w.backend._S[b] = 1.0
    w.probe_many(np.array([b]), 0, 1)
...
def test_llm_backend_redraw_is_offline_and_restorable():
    ...
    be.restore(snap)
    assert be.profiles == snap[0] and be.dir == snap[1]

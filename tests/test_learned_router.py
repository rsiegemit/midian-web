...
def test_knn_router_equals_flat_with_family_embedding(monkeypatch):
    w = World(60, 8, "specialist", 0.0, seed=3)
    names = list(w.families)
    def fam_onehot(texts):
        E = np.zeros((len(texts), len(names)), np.float32)
        for i, t in enumerate(texts):
            E[i, names.index(re.search(r"family (.+?) \(", t).group(1))] = 1
        return E
    monkeypatch.setattr(L, "embed", fam_onehot)
    flat, knn = FlatProbeArgmax(), KNNRouter()
    w.reset()
    flat.build(w.view(flat.needs), Budget(3))
    w.reset()
    knn.build(w.view(knn.needs), Budget(3))
    tasks = w.tasks(50)

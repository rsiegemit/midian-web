...
def python(code: str, timeout: float = 5.0) -> str:
    ...
    key = "tool:python:" + hashlib.blake2b(code.encode(), digest_size=16).hexdigest()
    return memo_call(key, lambda: _python(code, timeout))
...
# is one entry in RUN, one in HINT, one pattern in midian.backends.prompts.TOOL_RE.
HINT = {

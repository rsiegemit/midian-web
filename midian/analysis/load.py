...
(midian.methods.keys) and hands the frame to `prepare`, which adds the arm label (method + params, ALIAS'd to one name
per arm), the method group (from the method's `needs`), the framework accountings and the total-communication columns.
"""
import json
import os
import sys
...
def group_of(name):
    ...
    if name.startswith("fw_"):
        return "framework"
    if name.startswith("midian"):
        return "midian"
    if name in ("oracle", "random"):
        return "ceiling" if name == "oracle" else "floor"
    try:
        from ..methods import load_method
        needs = frozenset(load_method(name).needs)
    except Exception:  # optional dep or LLM-only file
        return "unknown"
    if not needs & {"probe", "reports"}:
        return "declared"
...
def load(grids):
    ...
    for g in grids:
        ...
        if os.path.isdir(f"{d}/rows.d"):  # refresh rows.csv from the per-row files
            consolidate(d)
        if os.path.exists(f"{d}/rows.csv"):  # old MIDIAN keys -> current
            frames.append(keys.normalize(pd.read_csv(f"{d}/rows.csv"), d))
        else:
            log(f"[analyze] no rows for grid {g!r} at {d}")
    if not frames:
        raise SystemExit(f"no rows found for grids {grids}")
...
def prepare(df):
    ...
    if legacy.any():
        ...
        r = np.array([json.loads(p).get("r", 10) for p in df.params])
        depth = np.ceil(np.log(df.n.to_numpy()) / np.log(r)).astype(int)
        ...
        if "total_comm_per_task" in df:
            df.loc[legacy, "total_comm_per_task"] = df.loc[legacy, "total_comm_per_task"].to_numpy() + depth[legacy]
    ...
    for c in per + BUILD[:3]:                              # pre-rewrite CSVs lack some counters
        if c not in df:
            df[c] = 0.0
    if "total_comm_per_task" not in df:
        df["total_comm_per_task"] = df[per].sum(axis=1)
    if "build_total_comm" not in df:
        df["build_total_comm"] = df[BUILD[:3]].sum(axis=1)
    return df

"""Grid runner (SPEC §7).   python -m midian.run --grid smoke [--seeds 1-3] [--methods a,b] [--workers N] [--dry-run]
...
def all_methods(backend):
    ...
    names = [m.name for m in pkgutil.iter_modules(midian.methods.__path__)
...
def main(argv=None):
    p = argparse.ArgumentParser("midian.run")
    ...
    log(f"[midian.run] grid={a.grid} units_todo={len(units)} rows_done={len(have)} workers={workers} out={out}")
    if a.dry_run:
    ...
    log(f"[midian.run] {consolidate(out)} rows -> {out}/rows.csv in {time.perf_counter()-t0:.0f}s"
